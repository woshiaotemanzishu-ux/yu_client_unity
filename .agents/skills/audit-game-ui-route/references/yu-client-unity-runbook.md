...
> 本手册仅服务最终验收。文中的“修改/修复”描述用于判定预期实现，不授权验收任务直接改业务代码、Prefab 或公共工具；验收发现缺陷后应停止并转交相应开发 Skill。
...
- 任一实现手段与老端最终表现冲突时，记录为需要修改 Unity 的缺陷。老端存在版本或状态分支时，先记录实际入口、版本、账号状态与时间点，不得挑方便复刻的变体。
