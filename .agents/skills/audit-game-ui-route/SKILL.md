...
4. 只有选中叶子的各阶段都通过，才标 `done` 并继续同层下一条线。不要为每个小修改都重打 WebGL。
5. 代码改动批量累计后构建 WebGL 壳；资源/Prefab 改动按发布手册决定是否构建内容包。
6. 部署后清缓存或加版本参数，重新登录并严格复走选中分支的全部步骤。
7. 截取修复后同状态画面，并恢复所有可恢复的账号设置。
