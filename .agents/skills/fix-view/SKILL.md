...
转换流水线(见 `/convert-module`)产出的 prefab + View 代码,抽查发现问题时用这个逐项修。**老客户端同账号、同状态的真实运行最终表现是验收事实源；当前可编辑 Prefab 只是 Unity 侧唯一视觉实现源。优先增量改 Prefab，其次改运行态状态/绑定代码，跨页面共性才动公共链路。已接管页面禁止重转、重烤或整页覆盖。**
...
| 一组同尺寸弧形/放射页签统一偏移半宽或半高 | Laya 的 `(x,y)` 表示子节点左上角，Unity 槽位却用中心 pivot；数值相同但参考角不同 | 先记录 `reference_corner/pivot`，把两端都换算成页面根左上角矩形；离散弧形位置直接保存为 Prefab 具名槽位，不强套 LayoutGroup。成就案例中 65×70 节点统一偏移 32.5/35 即属此类 |
| 竖排标签重叠，源 JSON 却是 `height=0` | Laya Text/Label 在运行时自动测量，设计 JSON 的零高度不是最终可见高度 | 从老端运行树采集最终 bounds，再把真实宽高和中心语义固化到 Prefab；覆盖折叠/展开、选中/未选中、红点有/无，不按零高度转换 |
| 简单属性/标签行仍重叠，固定坐标看似都对 | 转换器从空模板烤出了固定文字宽度，真实 TMP preferred width 未参与排版 | 背景、点击面和状态图留在布局外；仅把连续文字放入独立容器，用 `HorizontalLayoutGroup/ContentSizeFitter` 消费 preferred width 并垂直居中，验证短值、长值、正负号和多段文本 |
...
| 粒子数量、大小和方向都对，但透明绿图变成灰色实心圆片/色块 | 最终 RawImage 合成丢弃真实 RT alpha，或用 RGB 亮度重建 alpha；源纹理的透明像素仍可能保存非黑 RGB | 逐段保存“源 RGBA→粒子材质 blend→原始 RT RGBA→最终 Canvas”；检查透明像素 RGB 和最终合成 shader。按资源语义使用真实 alpha/预乘 alpha/additive，不得让一个全局亮度 alpha 模式解释所有 UI 特效；Editor 与 WebGL 都要同时间点验证 |
| 固定边界旧特效混合覆盖/Alpha/Additive，进入共享 RT 后已无法无损反推每实例合成 | 转换架构丢失了老端私有 RT 的最终 RGBA 语义；继续调全局 shader 会破坏同链正常资源 | 先尝试按合成语义拆原生通道；仍不可逆时，只对已证实资源从真实老端特效宿主采最终 RGBA 序列，不截页面。绑定旧端指纹/参数/尺寸/帧率/时长/哈希，图集无损保 Alpha 与透明 RGB；老端缓存源在 Unity 保持一份时钟到 N 个 presenter。逐帧字节门禁后仍回原路线验 Editor/WebGL Canvas、轨迹与清理 |
| 重复奖励粒子相位、随机形态或重叠节奏不符 | 把老端缓存后的“1 个动态源→N 个二维显示副本”误迁成 N 个独立粒子源；随机发射在共享 RT 中提前叠加；内外两层变换被合并 | 只有在轨迹/相位证据确实不符时才查 `use_cache/cache key`、源数、显示副本数和复制前后合成；这条不能替代透明度/颜色合成诊断，也不能用来解释数量、大小和方向已经一致的灰片问题 |
...
## 步骤
1. 读问题来源(diff 报告或用户描述),定位到具体节点 + 判断改哪层(上表)。位置类先写几何合同：`old_page_rect/unity_page_rect/reference_corner/pivot/runtime_size/parent_transform/state/text_bounds`；特效类先分清几何、模拟拓扑、原始 RT 和最终合成四层，禁止跨层猜根因。
...
4. **重新 diff**：2D 在同逻辑分辨率保存 old/unity/diff；弧形/折叠标签必须逐状态输出页面根矩形而非只验默认态。3D 不要求跨引擎逐像素相同，但必须有模型、无镜像翻转和明显错误角度，位置与比例大致正常且模型特效一致。
...
## 原则
- 老端最终画面、状态、时序、合成或生命周期与转换规则、Prefab 现状、Unity 架构或静态断言冲突时，一律调整 Unity 实现；禁止为维护通用方案而重新解释、降低或改写老端标准。用户运行证据只关闭明确观察到的缺陷范围，不自动扩张为整页或 WebGL 通过。
