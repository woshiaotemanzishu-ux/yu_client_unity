...
- 已经走配置读取的功能禁止在业务代码里硬编码兜底;宁可暴露缺配置/功能残缺,也要补真实配置、同步工具或读取器。
- **工具有问题随时优化**:发现工具缺陷/卡操作,先修工具再继续干活,不绕过、不手工凑;
  转换产物的问题优先修转换器/分析器的通用规则,不点杀单个产物。
- 业务行为对齐老客户端 = 查 yu_client TS/PHP 源码,禁止凭记忆猜。
- 端到端验证一个真实样本,再铺开;核心没跑通前不写防护/辅助代码。
- 架构、公共组件、工具链、资源流水线、关键链路或可复用疑难修复有变化时，必须按 `Docs/README.md` 在同一提交更新技术/经验文档并维护索引；禁止只把结论留在聊天、临时输出或外部记忆中。
