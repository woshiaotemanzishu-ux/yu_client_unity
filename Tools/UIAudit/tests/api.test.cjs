...
test('public root entry exposes versioned callable modules', () => {
  assert.equal(uiAudit.version.versionInfo().version, '1.1.8');
  ...
  for (const name of ['safeJson', 'runtimeTree', 'runtimeOverlay', 'selectorDiagnostic', 'canvasInput', 'popupPolicy', 'itemUse', 'protocolProbe', 'routeAssertions', 'runtimeProbes', 'serverReadiness', 'serverLifecycle', 'resourceToolPreview', 'preflight', 'session', 'report', 'routeRunner']) {
