...
namespace Shenxiao.Module.Core.Login
{
    ...
    public sealed class LoginLoadingView : LoginLoadingViewBind
    {
        ...
        /// <summary>
        /// 背景图:对标老端 LoadSuccess 里按 _preload_bg_id 取 load_bg{id}.jpg。
        /// id 数据来源是 LoginModel._preload_bg_id(真·运行时由预加载流程决定);这里没有该字段时
        /// 退回 prefab 烤进的默认背景(转换器「动态图默认填充」已塞过 load_bg*.jpg 第一张),不强行换图。
        /// </summary>
        private void RefreshBackground()
        {
            if (_img_bg == null) return;
            int bgId = ResolvePreloadBgId();
            if (bgId <= 0) return; // 用 prefab 烤进的默认背景
            _ = ResManager.SetImageAsync(_img_bg, $"resource/game/login/texture/load_bg{bgId}.jpg", nativeSize: false);
        }
