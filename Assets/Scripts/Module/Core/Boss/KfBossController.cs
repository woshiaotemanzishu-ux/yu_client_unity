...
namespace Shenxiao.Module.Core.Boss
{
    ...
    /// ②进场景族(47003/47004/47102/47103)遵循"失败显式回包/成功隐式(靠场景切换事件)确认"惯例,
    /// 严禁为"成功"专门等一个包;③47010 kf_server_allot 占位错误码不做自动重试轮询(老端本身也只是提示语);
    /// ④死号严禁发:47001(发送侧 C2S 死号)/47011(整链路死代码,组包函数零调用点)不注册。
    /// </summary>
