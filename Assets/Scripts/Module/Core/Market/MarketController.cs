...
namespace Shenxiao.Module.Core.Market
{
    ...
    /// 15122(老端活号 17 个中除已接的 15121 外的其余 16 个)。排除号 15103(协议定义缺失)/15104(服务端
    /// 搜索查询存活,但老端无 sender 且 handler 空消费)/15105(服务端推荐价查询存活,但老端同样无 sender
    /// 且空消费；服务端错误分支还在产 bug)/15107(P2P上架,
    /// do_handle 整段注释)/15110(P2P列表,注释+write缺)/15113(P2P红点,触发链依赖已注释的15107)——
    /// 六个排除号严禁注册/严禁发送,详见 Proto.cs 对应段落注释与 r18_server_market.md 台账。**服务端统一
    ...
    public sealed class MarketController : BaseController
    {
        ...
        protected override void Register()
        {
            ...
            // 自动循环 轮19 扩展:15100-15120/15122(除排除号)。15103/15104/15105/15107/15110/15113
        ...
        // ======================================================================================
        // 自动循环 轮19 扩展:15100-15120/15122 发送封装 + 协议处理(排除号 15103/15104/15105/15107/
