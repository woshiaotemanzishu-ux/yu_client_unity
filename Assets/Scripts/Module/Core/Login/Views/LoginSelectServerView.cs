    ...
    public sealed class LoginSelectServerView : LoginSelectServerViewBind
    {
        ...
        private void BuildTabs()
        {
            ...
            for (int i = 0; i < tabs.Count; i++)
            {
                ...
                var bind = tab.GetComponent<Shenxiao.Generated.UI.Login.LoginSelectServerTabItemBind>();
                if (bind == null) { GameLog.Error("Login", "tab 缺 LoginSelectServerTabItemBind,重跑回填"); continue; }
                bind._lb_name.text = label;
                int captured = area;
                UIUtil.AddClick(bind._img_bg, () => OnClickTab(captured));
        ...
        private void RefreshTabStates()
        {
            for (int i = 0; i < _tabs.Count; i++)
            {
                ...
                var bind = _tabs[i].GetComponent<Shenxiao.Generated.UI.Login.LoginSelectServerTabItemBind>();
                if (bind == null) continue;
                // 不能用 enabled 隐藏:Graphic 禁用后不接收点击;用 alpha=0,点击区保持有效
                bind._img_bg.enabled = true;
                bind._img_bg.color = selected ? Color.white : Color.clear;
                bind._lb_name.color = selected ? TAB_SELECTED : TAB_NORMAL;
            }
        }
        ...
        private void BindItem(GameObject item, LoginServerInfo server)
        {
            ...
            var bind = item.GetComponent<Shenxiao.Generated.UI.Login.LoginSelectServerItemBind>();
            if (bind == null) { GameLog.Error("Login", "服务器行缺 LoginSelectServerItemBind,重跑回填"); return; }
            bind._lb_server_name.text = string.IsNullOrEmpty(server.name)
                ? $"服务器 {server.id:0000}"
                : server.DisplayName;
            bind._lb_level.text = server.level > 0 ? "LV." + server.level : string.Empty;
            bind._img_career.enabled = server.career > 0;
            bind._img_tips.enabled = server.isNew;
            UIUtil.AddClick(bind._img_bg, () => OnClickItem(server));
        }
