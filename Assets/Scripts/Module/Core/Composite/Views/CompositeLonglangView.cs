...
{
    /// <summary> 天殒铸灵页(对标老端 composite/CompositeLonglangView.ts,CompositeView 标签内容)。降级:CompositeModel/协议未移植 → 红点/模板隐藏、列表空;动作按钮日志;无关闭(合成窗框统一关闭)。 </summary>
    public sealed class CompositeLonglangView : CompositeLonglangViewBind
    {
        ...
        {
            GameLog.Info("Composite", "天殒铸灵打开 → 待对接 CompositeModel(默认降级)");
        }
