...
{
    public sealed class ConRechargeItem : ConRechargeItemBind
    {
        ...
        public void SetData(CustomActivityModel.ActEntry info, CustomActivityModel.DetailReward reward)
        {
            ...
            if (_lb_title != null) _lb_title.text = string.Format("累计{0}天充值{1}元，可领取奖励！", day, gold / 10);
            if (_lb_progress != null) _lb_progress.text = string.Format("（今日已充值：{0}元）", current / 10);
