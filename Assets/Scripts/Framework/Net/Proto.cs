namespace Shenxiao.Framework.Net
{
    ...
    public static class Proto
    {
        ...
        public const int DUNGEON_HEART_SKILL_INFO = 61101;
        ...
        public const int WELFARE_CARD_LIST = 15901;
        ...
        /// <summary>龙纹物品详情。C2S goods_id:u64,location:u16；S2C location:u16 + dragon goods_list。</summary>
        public const int LUNG_GOODS_DETAIL = 18113;
        ...
        /// <summary>周一嘉礼个人开奖记录。C2S 严格空包；S2C: count:u16×{role_id:u64,role_name:string,type:u8,pool_id:u16,utime:u32,picture:string,picture_ver:u32,career:u16}。</summary>
        public const int MONDAYS_AWARD_PERSONAL_RECORDS = 17902;
        ...
        // 15011(旧他人基础装备查询):现役 19501→19502 已从同一 GoodsStatus/lib_goods_util 权威源返回相同九字段装备元组,
        //   并补齐法阵/仙灵及同服/跨服入口;恢复本号会制造第二条资料权威链。R569 已进 killlist。
        ...
        // 15028(简单物品合成):服务端会按规则及材料清单真实扣料、产物并回奖励;Unity 合成页尚缺配置、材料冻结、
        //   背包权威、单飞和结果展示闭环。R569 纳入 hard-negative,禁止先接孤立常量/sender/ACK。
        ...
        /// <summary>神纹副本阶段奖励领取情况显式查询。C2S dun_id:u32；S2C dun_id:u32,
        /// history_wave:u8,claimed_waves[u16×u8]。按副本保存完整原始快照；不自动请求、不派生领取状态。</summary>
        public const int DUNGEON_DRAGON_STAGE_REWARD = 61051;
        /// <summary>神纹副本快速出怪信息显式查询。C2S 严格空包；S2C quick_count:u16,
        /// total_quick_count:u16,next_quick_time:u32。每包整体替换单份原始快照；不自动请求、不派生倒计时。</summary>
        public const int DUNGEON_DRAGON_QUICK_INFO = 61053;
        /// <summary>神纹副本临时技能数量查询/推送。C2S 严格空包；S2C
        /// skill_list[u16×{skill_id:u32,num:u16}]。每包整体替换有序原始快照，保留重复项。</summary>
        public const int DUNGEON_DRAGON_SKILL_INFO = 61055;
        /// <summary>神纹副本跳关奖励通知（S2C-only，无 C2S 请求）。S2C wave:u32,
        ...
        /// <summary>高级经验副本跳关进入通知（S2C-only，无 C2S 请求）。S2C wave:u32,
        /// history_wave:u32,exp:u64。奖励和经验由服务端在推送前落账；本端保存独立于 61059 的原始快照。</summary>
        public const int DUNGEON_ADVANCED_EXP_JUMP_INFO = 61061;
        ...
        // 其余 16 个。排除号 15103(协议 read/write 双缺但业务层活,发了也拿垃圾响应;read no_match 后更
        // 可能网关静默丢弃——pt_151:read 全仓无调用点[全仓 grep pt_151: 仅命中 :write,无一处 :read],
        // 网关是独立二进制,不在本仓库源码树内,无法在此继续追踪确证)/15104(搜索查询服务端存活,但老端
        // 无 sender 且 handler 解包即丢弃)/15105(推荐价查询主链存活,但老端同样无 sender 且空消费；服务端
        // 错误分支另有在产 bug:1元素vs3字段定义)/
