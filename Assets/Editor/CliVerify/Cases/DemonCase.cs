...
namespace Shenxiao.EditorTools
{
    public static class DemonCase
    {
        ...
        {
            DemonController c = DemonController.Instance; DemonModel m = DemonModel.Instance; bool was = c.IsInitialized; byte oldOpen = m.OpenState; bool oldHas = m.HasData; bool oldHasFetters = m.HasFettersData; var old = new List<DemonModel.Entry>(m.Demons); var oldFetters = new List<uint>(m.Fetters); FieldInfo fi = typeof(DemonController).GetField("s_outboundIntercept", SF); object oldIntercept = fi?.GetValue(null);
            try
            {
                c.Init(); m.Reset(); MethodInfo on = typeof(DemonController).GetMethod("On18301", F); MethodInfo onFetters = typeof(DemonController).GetMethod("On18303", F); var h = typeof(NetManager).GetField("_handlers", SF)?.GetValue(null) as IDictionary; bool pass = fi != null && on != null && onFetters != null && h != null && h.Contains(Proto.DEMON_INFO) && h.Contains(Proto.DEMON_FETTERS) && !h.Contains(18302); for (int id = 18304; id <= 18317; id++) pass &= !h.Contains(id); void Check(string tag, bool ok) { Debug.Log("CLIVERIFY demon " + tag + " ok=" + ok); if (!ok) pass = false; } Check("seams/register", pass); if (!pass) return 3;
                var frames = new List<byte[]>(); fi.SetValue(null, new Func<byte[], bool>(f => { frames.Add(f); return true; })); c.RequestStartup(); Check("startup exact empty frames", Frames(frames, Proto.DEMON_INFO, Proto.DEMON_FETTERS)); frames.Clear();
                ...
                byte[] fetters = new CliVerify.Pkt().H(4).I(11).I(4000000000L).I(11).I(12).Bytes(); var f1 = new NetReader(fetters, 0, fetters.Length); onFetters.Invoke(c, new object[] { f1 }); Check("fetters dedupe/read-to-end/no-outbound", f1.Remaining == 0 && m.HasFettersData && m.Fetters.Count == 3 && m.Fetters[0] == 11 && m.Fetters[1] == 4000000000U && m.Fetters[2] == 12 && m.HasFetter(4000000000U) && frames.Count == 0);
                byte[] fettersSecond = new CliVerify.Pkt().H(1).I(13).Bytes(); var f2 = new NetReader(fettersSecond, 0, fettersSecond.Length); onFetters.Invoke(c, new object[] { f2 }); Check("fetters whole replace", f2.Remaining == 0 && m.Fetters.Count == 1 && m.HasFetter(13) && !m.HasFetter(11) && frames.Count == 0);
                byte[] fettersEmpty = new CliVerify.Pkt().H(0).Bytes(); var f3 = new NetReader(fettersEmpty, 0, fettersEmpty.Length); onFetters.Invoke(c, new object[] { f3 }); Check("fetters empty replaces", f3.Remaining == 0 && m.HasFettersData && m.Fetters.Count == 0 && frames.Count == 0);
                c.Dispose(); Check("dispose reset", !c.IsInitialized && !m.HasData && !m.HasFettersData && m.OpenState == 0 && m.Demons.Count == 0 && m.Fetters.Count == 0); Debug.Log("CLIVERIFY demon VERDICT pass=" + pass); return pass ? 0 : 3;
            }
            finally { if (c.IsInitialized) c.Dispose(); m.Reset(); if (oldHas) m.Replace(oldOpen, old); if (oldHasFetters) m.ReplaceFetters(oldFetters); if (was) c.Init(); if (fi != null) fi.SetValue(null, oldIntercept); }
        }
