    ...
    public static class JjcRecordsCase
    {
        ...
        static int RunSync()
        {
            ...
            try
            {
                ...
                for (int p = 28000; p <= 28018; p++) Check("registration " + p, handlers.Contains(p) == (p >= 28001 && (p <= 28004 || p == 28009)));
                ...
                try {
                    ...
                    m.Apply28001(1,2,3,4,5,6,7,8,true,9,new List<int>()); m.Apply28004(10,11,12,13); m.Apply28009(3, new List<JjcModel.RecordVo>{ new JjcModel.RecordVo{RoleId=1} }); frames.Clear(); EventDispatcher.Emit(GlobalEvent.EVT_GAME_START); Check("game start clears/no 28009", !m.HasInfo&&!m.HasTimesInfo&&!m.HasChallengeRecords && Frames(frames,28004,28001));
                    ...
                    c.Dispose(); frames.Clear(); EventDispatcher.Emit(GlobalEvent.EVT_GAME_START); bool removed=true; foreach(int id in new[]{28001,28002,28003,28004,28009})removed&=!handlers.Contains(id); Check("dispose",removed&&frames.Count==0&&!m.HasInfo&&!m.HasRivals&&!m.HasChallengeResult&&!m.HasTimesInfo&&!m.HasChallengeRecords);
                } finally { fi.SetValue(null,prev); }
                return pass?0:3;
            } finally { if(c.IsInitialized)c.Dispose(); m.Clear(); if(oldInfo)m.Apply28001(rank,history,reward,combat,hp,num,refresh,honour,oldReward,pet,oldBreaks); if(oldRivalsFlag)m.Apply28002(oldRivals); if(oldResultFlag)m.Apply28003(oldWin?1:0,oldResult); if(oldTimes)m.Apply28004(timesErr,left,at,can); if(oldLoaded)m.Apply28009(oldErr,old); if(was)c.Init(); }
        }
