...
namespace Shenxiao.EditorTools
{
    public static class FriendInviteLevelCase
    {
        ...
        private static int RunCore()
        {
            ...
            bool oldHas = m.HasLevelInfo, oldHelp = m.HasHelpInfo, oldShare = FriendInviteModel.ShareOpen;
            ...
            bool old34000 = handlers != null && handlers.Contains(34000), old34001 = handlers != null && handlers.Contains(34001), old34005 = handlers != null && handlers.Contains(34005), old34006 = handlers != null && handlers.Contains(34006);
            object old34000Ref = old34000 ? handlers[34000] : null, old34001Ref = old34001 ? handlers[34001] : null, old34005Ref = old34005 ? handlers[34005] : null, old34006Ref = old34006 ? handlers[34006] : null;
            try
            {
                c.Init(); m.Reset(); var on = typeof(FriendInviteController).GetMethod("On34006", NP); var onHelp = typeof(FriendInviteController).GetMethod("On34005", NP);
                var onInfo = typeof(FriendInviteController).GetMethod("On34001", NP);
                pass = on != null && onHelp != null && onInfo != null && intercept != null && handlers != null
                    && lastLevel != null && roleReady != null && eventHandlers != null && icons != null && boxes != null;
                for (int p = 34000; p <= 34012; p++) pass &= handlers.Contains(p) == (p == 34000 || p == 34001 || p == 34005 || p == 34006);
                var frames = new List<byte[]>(); intercept.SetValue(null, new Func<byte[], bool>(f => { frames.Add(f); return true; }));
                c.RequestLevelInfo(); pass &= Frames(frames, 34006); frames.Clear(); c.RequestHelpInfo(); pass &= Frames(frames, 34005); frames.Clear(); c.RequestStartup(); pass &= Frames(frames, 34001, 34005, 34006);
                ...
                role.MarkBaseInfoReady(); role.Level = 654321; lastLevel.SetValue(c, 0); frames.Clear(); EventDispatcher.Emit(GlobalEvent.EVT_ROLE_INFO_UPDATE); pass &= Frames(frames, 34001, 34005, 34006);
                ...
                Invoke(onHelp, c, new CliVerify.Pkt().H(0).H(0).H(0).Bytes(), out helpRemain); pass &= helpRemain == 0 && m.HasHelpInfo && m.HelpCount == 0 && m.HelpRewards.Count == 0 && m.HelpInviteEntries.Count == 0 && m.HasLevelInfo && m.LevelInviteEntries.Count == 0;
                // Dispose 必须从三个非空 slice 出发验证全清，不能借前面的空快照假绿。
                Invoke(onInfo, c, InfoPacket(), out infoRemain);
                Invoke(on, c, PacketSingle(), out remain);
                Invoke(onHelp, c, HelpSinglePacket(), out helpRemain);
                pass &= infoRemain == 0 && remain == 0 && helpRemain == 0 && m.HasLevelInfo && m.LevelInviteEntries.Count == 1
                ...
                pass &= !handlers.Contains(34000) && !handlers.Contains(34001) && !handlers.Contains(34005) && !handlers.Contains(34006) && !m.HasLevelInfo && !m.HasHelpInfo && m.LevelInviteEntries.Count == 0
            ...
            finally
            {
                ...
                if (handlers != null)
                {
                    handlers.Remove(34000); handlers.Remove(34001); handlers.Remove(34005); handlers.Remove(34006);
                ...
                bool restored = c.IsInitialized == was && m.GetStatus == oldGet && m.RecoverTime == oldRecover && m.DailyCount == oldDaily && m.TotalCount == oldTotal
                    && m.HasLevelInfo == oldHas && SameEntries(m.LevelInviteEntries, oldEntries) && m.HasHelpInfo == oldHelp && m.HelpCount == oldHelpCount && SameRewards(m.HelpRewards, oldHelpRewards) && SameEntries(m.HelpInviteEntries, oldHelpEntries) && FriendInviteModel.ShareOpen == oldShare
                ...
                if (handlers != null) restored &= handlers.Contains(34000) == old34000 && handlers.Contains(34001) == old34001 && handlers.Contains(34005) == old34005 && handlers.Contains(34006) == old34006
                    && (!old34000 || ReferenceEquals(handlers[34000], old34000Ref)) && (!old34001 || ReferenceEquals(handlers[34001], old34001Ref)) && (!old34005 || ReferenceEquals(handlers[34005], old34005Ref)) && (!old34006 || ReferenceEquals(handlers[34006], old34006Ref));
        ...
        private static bool Frames(List<byte[]> f, params int[] ps) { if (f.Count != ps.Length) return false; for (int i = 0; i < ps.Length; i++) if (f[i].Length != 6 || f[i][0] != 0 || f[i][1] != 6 || f[i][2] != 3 || f[i][3] != 232 || f[i][4] != (byte)(ps[i] >> 8) || f[i][5] != (byte)ps[i]) return false; return true; }
