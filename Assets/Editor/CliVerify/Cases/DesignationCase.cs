...
namespace Shenxiao.EditorTools
{
    public static class DesignationCase
    {
        ...
        {
            DesignationController c = DesignationController.Instance; DesignationModel m = DesignationModel.Instance; bool was = c.IsInitialized; uint oldCurrent = m.CurrentUsedId; var oldEntries = new List<DesignationModel.Entry>(m.Entries); bool oldHas = m.HasData; FieldInfo fi = typeof(DesignationController).GetField("s_outboundIntercept", SF); object oldIntercept = fi?.GetValue(null);
            try
            {
                c.Init(); m.Reset(); MethodInfo on = typeof(DesignationController).GetMethod("On41101", F); var h = typeof(NetManager).GetField("_handlers", SF)?.GetValue(null) as IDictionary; bool pass = fi != null && on != null && h != null && h.Contains(Proto.DESIGNATION_LIST) && !h.Contains(41100) && !h.Contains(41102); void Check(string tag, bool ok) { Debug.Log("CLIVERIFY designation " + tag + " ok=" + ok); if (!ok) pass = false; } Check("seams/register", pass); if (!pass) return 3;
            ...
            finally { if (c.IsInitialized) c.Dispose(); m.Reset(); if (oldHas) m.ReplaceData(oldCurrent, oldEntries); if (was) c.Init(); if (fi != null) fi.SetValue(null, oldIntercept); }
        }
