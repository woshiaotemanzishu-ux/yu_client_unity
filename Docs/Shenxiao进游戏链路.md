...
### 2.2.2 GAME_START 声音就绪门槛（2026-08-06）

`GameEntryFlow.CompleteGameStartAsync` 在派发 `EVT_GAME_START` 前会等待 `LegacyPreloadService.PreloadGameStartAsync`。声音也属于这个加载阶段的首屏闭包：与主界面、当前地图和主角资源并行预热当前场景唯一 BGM、当前职业展示/升级/跳跃/胜负语音、当前职业技能配置音和 3 个高频 UI 音效。世界揭开后 `SceneController.On12005` 仍按服务器权威场景选曲，但常规首图应直接命中驻留 Clip，不得等画面已经可操作后才开始下载音乐。

预热范围必须由当前角色、当前场景与当前职业技能配置生成，禁止全量加载 310 个声音或其他职业。预热失败采用 fail-soft：记录具体地址并允许真实播放点冷加载兜底，不能阻断协议主链，也不能播放猜测的场景音乐。音频驻留引用与普通播放引用分开计数，切曲、停止、运行时销毁和旧异步请求后到都必须保证恰好释放一次。
...
- `12002/12003` 的角色块可能包含主角自身；主角块只同步 `RoleModel` 的主角状态，随后必须结束分支。
- `SceneManager.AllRoles` 只保存其他玩家/假人，不得保存当前 `RoleModel.RoleId`。战斗、移动和场景表现均依赖这条边界。
- 遍历 `AllRoles` 创建称号等表现时仍须防御性跳过主角同 ID，避免热重载或旧脏表再次创建一份停在进场坐标的对象。
