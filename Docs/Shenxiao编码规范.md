...
4. 写代码 / 写工具 / 写 Schema
5. 按 [Docs 文档入口](README.md) 判断是否触发技术文档/经验文档更新
6. 自检清单（见 §十一）
7. 报告改了哪些文件、是否新增了 asmdef 引用、是否引入新依赖、更新了哪些文档
```

**工具优先**：工具是地基。使用中发现工具问题（缺陷、操作繁琐、报告不准）随时优化，
先修工具再继续业务；禁止绕过工具手工凑结果——手工结果会在下次工具重跑时丢失。
...
- 从合并模块 prefab 独立复用单个子 View 时，实例加载后必须先把全部**最外层同级** `BaseView` 设为 inactive，
  再只 `Show()` 当前归属的 View；关闭时还要失活该模块实例根。嵌套在目标 View 内的 Item/View 展示组件随父窗口管理，
  禁止用无层级过滤的 `GetComponentsInChildren<BaseView>()` 把它们单独关掉。也禁止依赖 Prefab 的默认 active 状态，
  否则转换快照中的同级页面会泄漏，或标题图标、列表项等嵌套展示会无故消失。
- 模态详情的遮罩、单板/双板结构、同级次序和对比坐标必须直接保存在 Prefab；业务代码只切显隐并绑定关闭语义。
  遮罩必须覆盖底层页面的关闭和导航点击；装备对比必须保持两份独立详情，不得压成一个拼接文本面板。
...
- 复合按钮只能保留一个顶层点击面；`bg/icon/lock/CD/文字` 等装饰 `Graphic` 必须
  ...
  覆盖一次真实 Prefab 的 `GraphicRaycaster → PointerClick`，直接调用点击方法不能替代。固定 HUD 入口若
  可见子图本身就是玩家命中面，`Button` 必须挂在该可见 `Graphic`，不得只给外层空 `_box_*` 动态补透明点击面。
...
[ ] 已按 Docs/README.md 判定文档影响；需要沉淀时，文档与实现同提交
[ ] 报告：动了哪些文件、为什么
```
...
文档分类、权威入口、命名和更新门槛统一见 [Docs/README.md](README.md)。以下情况不能只写代码：

- 新增或改变架构、公共组件、Editor 工具、资源转换/导入、构建和发布链路；
- 改变协议语义、登录/进游戏/主界面/场景等关键运行链路；
- 修复经过多轮定位、根因可复用、后续容易再次踩中的问题；
- 引入需要人工配置、Unity 菜单操作或特殊验收步骤的能力；
- 里程碑、范围、风险或 blocker 的状态发生变化。

交付要求：

1. 优先更新已有专题文档；没有合适文档时新建 `Docs/{模块}-{主题}.md` 或 `Docs/{模块}-经验与排障.md`。
2. 新文档必须加入 `Docs/README.md`；已验证进度写入 `Shenxiao实施进度.md`，禁止记录未经验证的“应该可用”。
3. 技术文档至少说明背景、最终方案、边界/非目标、关键文件、配置或操作、验证方式；经验文档至少说明症状、根因、证据、正确处理和禁止做法。
4. 文档与实现放在同一提交。形成长期硬约束时，同时更新 `AGENTS.md` 或本规范。
5. 纯错字和无行为变化的机械修改可不更新专题文档，但提交说明或最终报告必须写明原因。
6. 聊天记录、`output/`、`Temp/` 和外部 AI memory 只能作为取证材料，不能替代仓库内文档。
