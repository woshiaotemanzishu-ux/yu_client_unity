...
- `VipController.RequestStartup` 先清整个VIP模型，再严格发送三个6字节空帧 `45000 -> 45004 -> 15800`。跨天重拉同一安全读侧子序列，但不预清已有快照，避免无回包时丢状态；15901福利卡继续只允许显式查询，15803尚未迁移。
...
- `45005` 激活与 `45006` 超时只保存各自最后一份双u8原始通知，绝不直接改45004。45006收到后严格重拉一次45004，让服务端全量成为唯一卡表权威；45000、45004、45005、45006、15800/15801与15901切片互不交叉清理。
- 45001/02领奖、45003购买、45007领免费卡和45008隐藏VIP都会发奖、扣货币或持久化/广播角色状态，必须随UI、配置、背包货币、奖励和提示闭环整体迁移。当前不改Role/Figure，不接红点、倒计时、自动购买或本地奖励。
