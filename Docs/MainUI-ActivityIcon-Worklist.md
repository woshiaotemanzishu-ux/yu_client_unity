...
## 结论
- **2026-08-04 现状纠正**：`612xx` 是限购商城专属入口族。虽然配置为 loc2/3，老端会在排版末尾统一退出活动网格并把入口/特效留给底部商城；Unity 分组槽位重构曾漏掉此分流，导致 `61207`“圣衣秘境”误显，现已在 `CollectOwnedIconGroups` 恢复全族过滤。不得把“服务端有活动 + 配置 loc2”当作应显示依据，也不得只特判当前变体 `61207`。
...
| 🟡部分 | FirstRechargeController | `159`=首充(loc4) | 15905, 15906, 15907, 15908 | Assets/Scripts/Module/Core/FirstRecharge/FirstRechargeController.cs + FirstRechargeModel.cs exist. Ported: protocol handlers 15905/15906/15908、首充横幅的配置续体与 timer generation/Dispose 隔离；15907 仅在当前有效横幅到期后去重通知。面板/领取交互仍待完整迁移。 |
