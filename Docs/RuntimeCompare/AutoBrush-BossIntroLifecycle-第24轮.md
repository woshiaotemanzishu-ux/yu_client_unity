...
## 2026-07-29：文字消失后循环底纹仍残留

- **现象**：`大妖来袭` 字体图片已经结束，橙色流体底纹仍单独保留约 0.4 秒，随后才随固定 1.5 秒计时销毁。
- **根因**：昨天只把正常生命周期从错误的 3 秒兜底收回到老端调用上限 1.5 秒，但导入后的主体动画 `UI_2103.anim` 实际 `m_StopTime=1.083s`；`liutizuo/liutiyou` 是持续发射的循环粒子，不会自行跟随文字片段停止。
- **修复**：`UIEffectStage.Handle` 记录实例内最长 Legacy Animation 片段；`BossBornEffectPlayer` 取“配置上限与真实片段时长的较短值”，在 1.083 秒释放整个 Handle，再开始遮罩滑出。3 秒仍仅用于资源加载/回调异常兜底。
- **回归**：专项必须同时断言真实资源片段为 `1.083±0.002s`、组合演出解析时长等于该片段，防止以后又退化成“文字先消失、循环底纹后消失”。
