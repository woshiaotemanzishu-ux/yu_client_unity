...
## 当前阶段：按需首转，Prefab 精修落袋（2026-07-30）

- LayaUI 批量转换器现在只承担**首次导入**：业务首次需要某个尚无可编辑 Prefab 的界面时，按需转换该界面或最小依赖组，不再提前批量推进整个模块。
- 初始产物随后进入「神霄/重构UI 生成器」完成骨架补齐、公共升级和 Prefab 落袋；进入精修后，位置、尺寸、锚点、图片、字号、颜色、间距及 LayoutGroup 参数以当前 Prefab 为唯一视觉事实源。
- 已落袋界面禁止靠重跑批量转换修复。单页面问题直接增量修改其 Prefab；多个面板共用的问题修改共享 Prefab、公共 View 或可重复执行的公共 Prefab 精修器。
- 页面专用 Creator 只允许在首次成形前使用，人工接管或验收后从重建注册表移除。公共精修器不得重建业务页面树，只能幂等修改明确的共享 Prefab，并在保存后自验关键参数。
