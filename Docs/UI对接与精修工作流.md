...
从 2026-08-11 起，长页面使用 `.agents/skills/audit-game-ui-route/scripts/route_timing.py` 原子记录阶段和事件，禁止最后凭聊天时间线反推。主动工作与环境等待分开，重复失败使用同一 fingerprint；历史候选清单、共享证据或热会话带来的节省写入 `estimated_saved_ms`。UIAudit report 同时记录 preflight、阶段、步骤动作、采样等待和配置 settle；其中 `fixedWaitSavingsCeilingMs` 只是“若被真实 ready 探针替代”的上限，不能自动删除视觉时序要求。

用户明确确认完成且要求不再重复动的范围，登记在 `Tools/UIAudit/policies/completed-scopes.json`。相邻页面可以从该页过境，但 preflight 默认拒绝直接运行其 route；只有新的用户/真实运行证据或已证明的共享影响，才允许携带精确 `scope.reopen` 证据重开。这样既避免每次从“角色-人物”等已稳定父页重新精修，也不会阻止新证据精确回卷。
