...
- 最新完成轮次：R591。复核并机器化TreasureMap 203族两个当前活缺口：20301会首次生成并持久化地图坐标，旧端以其编排跨场景、寻路、停自动战斗和2秒采集；20302严格校验实例/类型/场景/距离后真实删图、写次数、发奖或邮件、推进活动并重置剩余地图坐标。
- 20301/20302进入hard-negative；20300原始错误、20303开奖记录及唯一显式`RequestDrawLog()`保持不变，baseline只补人工说明且历史0/4计数冻结。双常量故障注入被F同时拒绝；最终`coverage_20260802_233325.md`为A～M全PASS、F=97条、G=174条、治理分类`81+97+153=331`，raw指标保持`1256/331/20/77.5%`。
