...
- 最新完成轮次：R568。coverage新增活缺口治理分类汇总，当前331号显式分为有效killlist 79、有效硬负约束20、未落机器清单232，合计331。
- 新增M段独立按同次扫描逐号重建汇总四行；K继续守85族/331号逐族明细，M守跨族分类总量，禁止沿用历史常量、分类互换、重复计数或只让合计看似正确。
- `80+20+231=331`负向探针保持合计不变，M仍精确报告期望/实际分类差异。恢复后的`coverage_20260802_202150.md`为`registered=1256/liveDefined=1468/liveGap=331/errorExit=20/active=1137/1468=77.5%`，A～M全PASS、`EXIT 0`，正式baseline与治理清单未改。
