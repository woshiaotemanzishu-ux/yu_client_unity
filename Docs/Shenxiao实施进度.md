...
- 最新推进轮次：R598。复核并机器化Kaifu 420族两个当前未分类事务：42002扣完整投资成本或升级差价、写投资DB，可激活称号/发购买奖励并触发红包、计数、活动与VIP，月卡还由充值事件主动调用同一链；42003按等级/天数/登录/前置/每日条件校验，先持久化领取态与时间，再背包或邮件发差额奖励。
- 42002/03进入hard-negative，完整分类本族两个当前活缺口；现有42000/01/04与启动/等级/跨天链保持不变，baseline只补人工说明且历史2/3计数冻结。正向`coverage_20260803_004134.md`通过；双常量探针在`coverage_20260803_004311.md`被F同时拒绝；恢复后的`coverage_20260803_004508.md`为A～M全PASS，治理分类`81/116/134`，raw仍为`1256/331/20/77.5%`。
