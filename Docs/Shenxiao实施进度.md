...
- 最新完成轮次：R475，藏宝图家族 S2C-only 20300 统一原始错误出口已接入；每包整体覆盖独立 `code:u32` 切片，零值与极值有效，并与既有 20303 开奖记录快照双向隔离。不公开 20300 请求或复刻老端 Toast/Reset；20301 使用/寻路与 20302 消耗/发奖继续等待场景、采集、背包和奖励展示完整事务链。
