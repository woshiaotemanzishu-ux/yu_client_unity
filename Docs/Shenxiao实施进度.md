...
- 最新裁决轮次：R216，40218 的空包 C2S 已迁移，但专属 S2C writer 无调用点且失败统一走40200，故保持“不注册40218”的现有实现；最新有效实现仍为R214的 MiniGame 39900 原始错误快照。
- Unity ProtocolCoverage：`registered=1021`，`liveDefined=1468`，`liveGap=569`，未注册族错误出口16个；真实活协议覆盖 `899/1468=61.2%`，A～E 治理断言全通过。
