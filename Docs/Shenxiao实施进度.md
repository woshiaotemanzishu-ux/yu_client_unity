...
- 最新完成轮次：R587。复核并机器化Eternity 279族最后两个尚未落机器清单的活缺口：27902是真实跨服入场事务，会校验开放/人数/归属、写参与表、触发任务/至尊VIP/活动日历并切换场景及PK；27903是真实退出事务，会切回默认场景并清本地/跨服参与表。
- 27903当前还存在服务端确定缺陷：`pp_kf_sanctum`误用`pt_284:write(27903,...)`，最终只发`cmd=0`空包，客户端禁止伪造正确ACK。27902/03现进入hard-negative，与R540既有27904-09共同覆盖279族全部八个当前活缺口；27900/01保持不变，baseline只补人工说明且历史0/10计数冻结。双常量故障注入被F同时拒绝；最终`coverage_20260802_225735.md`为A～M全PASS、F=81条、治理分类`80+81+170=331`，raw指标保持`1256/331/20/77.5%`。
