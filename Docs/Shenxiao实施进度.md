...
- 最新推进轮次：R622。按R505及逐号复核的DungeonSec裁决，把61108阶段领奖、61109扫荡、61110场景怪物进度、61112目标领奖、61114每日领奖、61116解锁和61118塔大奖七条业务边界补入hard-negative；与61103/07/19 killlist完整分类本族十个当前活缺口。
...
## 2026-07-29：人物角色与设置面板老端对标

- **人物角色**：恢复老端 13 项基础属性与 31 项极品属性顺序、切换标题、原始整数/百分比格式和双列排版；长属性名通过 Prefab 内 HBox 自适应，不再覆盖数值。已接模块入口进入真实页面，未转换入口统一走路由提示，不再静默无响应。
- **设置面板**：纠正同名但内容错误的设置贴图，按老端恢复头像区、四条滑杆、自动拾取、任务/坐骑/降神设置、10 项屏蔽项、底部五个操作及双页签；动态块只控制行显隐，位置完全由 Prefab 布局驱动。
- **验证状态**：`RoleSettingPanelCase` 覆盖设置静态布局/贴图尺寸、人物 Prefab 结构、13/31 项属性及显示格式，返回 `pass=True`；`SettingPkCase` 覆盖 10202 设置快照、13012 PK 状态、四滑杆、3 项拾取、10 项屏蔽、任务项及战斗模式弹窗，返回 `pass=True`。最终 `RoleModule.prefab`、`SettingModule.prefab` 均无 Missing Script。
- **专题文档**：[人物与设置面板对标-经验与排障.md](人物与设置面板对标-经验与排障.md)。
...
- **神装/共鸣 blocker**：R570已将`15218/15221/15222`纳入hard-negative。15218虽有`Godequipbuild`转换Prefab，但没有业务View且缺完整等级/资格配置；`EquipSuitBaseView`仍无可编辑Prefab，老端`127.0.0.1:8090`运行快照服务不可达。三号都会改变真实资产/属性，15222还可能由换装链主动下发，因此未接裸协议、raw handler或临时Toast；共鸣按钮继续关闭射线，等待完整模块与资产链闭环。
...
## 2026-08-02：Adventure 42704 晚增负约束补漏（R546）
...
- **漏验收事实**：用户完整提交改名后，当前设置面板没有立即刷新角色名，只有退出后才更新；更换头像虽能跳转，但加载至少 5 秒，且 Unity 显示城楼背景，而老端截图为角色居中+分类库格。继续逐页对照后确认两张截图不是同一页：Unity 开的是外层“发饰”，老端截图是外层“装扮”的内层“头像”；真正问题是入口用数字索引开错外层页签，不是整个 Fashion 页面版本漂移。
...
## 2026-08-03：设置→更换头像分支深度闭环

- **路由与页面**：设置入口改为 `SettingFlow.Close()→FashionFlow.OpenDress(DressView.HeadType)`，稳定落在外层“装扮”、内层“头像”。外层统一为时装/发饰/装扮/套装四页签；在现有 `DressModule.prefab` 上增量补齐四列列表、预览、属性、材料和唯一可见点击面，运行时代码只填数据与事件。
...
## 2026-08-12：角色尺寸改回源模型标准（1400 参照）

- **口径变更**：美术统一模型导出单位与大致体量，1400 作为参照；模型之间允许合理设计差异。2026-08-03 的 1201/1213 比例补偿保留为历史记录，但不再是当前运行口径。
- **程序修复**：主场景移除新模型 `0.85` 特判；`ArtModelStager` 只应用落点平移，不再乘 `landingScale`；导入器停止按 2.33 目标身高生成缩放。现有角色动作 Prefab 的 `landingScale`、1201 的 `attachmentSpaceScale` 及装配档案全部回到 `1`。
- **门禁更新**：`CliVerify.RoleAttachmentSpace` 改为检查角色与附件倍率全为 `1`；场景渲染和角色特效用例使用默认正常尺寸。页面构图用的展示比例仍由各页面自行控制，不属于模型源尺寸补偿。
...
- **第四次真实回卷**：玩家截图确认内置 `RawImage` 已恢复出帧，但真实 `63/64/64/66` 被画出额外竖笔和邻字残片。根因不是 UV，而是 2026-08-04 全局换字后的十张 `fight_font_*` PNG 把实心笔画画出原 FNT 槽位：错误图有 77 个数字槽在 Alpha 224 阈值下触及左右边界，替换前配套图为 0。现只恢复这十张专用战斗 PNG，并同步修正老端 CDN 事实源；其余字体、FNT、TMP 资产和 Addressables 不动。专项新增逐数字槽 Alpha 边界门禁；Core/Editor 离线编译分别为 97/2 个既有 warning、均 0 error，未占用用户 Unity，玩家同场“可读→开角色→关闭→仍可读”仍待复验。详见 [老端图片字体全局迁移 §8](RuntimeCompare/BitmapFont-全局迁移-20260805.md#8-2026-08-06-第四次用户复查替换图跨-fnt-槽串字)。
...
- **确定性验证**：`node --test Tools/UIAudit/tests/*.test.cjs` 为 35/35 通过，覆盖公共根 API、循环/共享引用、弹窗顺序与危险节点、三来源归一化、ItemUse 精确身份/稳定帧/队列/显式模式、协议 required/forbidden/公共读策略/自定义写包 trace、URL readiness 与错误分类、失败不创建 run/不构造 session、后台服务 owner 身份、dataIdentity、宽屏 Canvas 换算、条件分支、正负存在、几何/滚动/裁剪/末项、声音计数与 RT 非透明稳定帧。本增量未启动浏览器、未登录游戏或执行页面验收。
