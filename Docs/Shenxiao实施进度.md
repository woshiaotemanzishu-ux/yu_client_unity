...
- 最新完成轮次：R560。B段现要求179个家族的冻结`liveGap`全部非负、分项总和精确等于顶层`totalLiveGap`，且顶层历史活缺口位于`0..totalLiveDefined`；禁止单改分项或汇总制造自相矛盾的防退基线。
- 当前179族历史活缺口分项总和为697，与顶层697一致。只把100族从0改为-1的反向探针被B同时报为`家族liveGap为负:100`与`汇总696!=历史总量697`，总判决失败。
- 恢复后的`coverage_20260802_191606.md`为`registered=1256/liveDefined=1468/liveGap=331/errorExit=20/active=1137/1468=77.5%`，A～H全PASS、`EXIT 0`，B显示家族冻结计数/逐号清单及历史活缺口分项/总量均一致。正式baseline未改。
