...
- 最新完成轮次：R491，复核远古奥术 `21101-21104` 全家族并补齐机器KILL：21101在老端启动查询后仅解包丢弃，奥术页签/视图/模型消费均被注释；21102升级、21103突破、21104选核心虽有真实服务端写事务，但老端无任何可达发送入口。Unity继续零常量、零注册、零请求、零模型/UI，不从残留结果回调反造功能。
- Unity ProtocolCoverage：`registered=1085`，`liveDefined=1468`，原始`liveGap=505`，未注册族错误出口10个；运行时真实注册覆盖 `963/1468=65.6%`。killlist只治理“有证据的不迁移缺口”，不伪增运行时覆盖；A～E 治理断言全通过。
