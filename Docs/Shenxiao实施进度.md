...
- 最新完成轮次：R494，接入龙语622家族五个安全读侧切片：GAME_START清状态后只空发62201装备/强化有序全量；62207总评分、62208按goods_type_id的最后raw套装预览与62209当前套装全量均保持显式查询，62200只保存S2C-only raw错误。服务端真实套装项num为u16、62208 code为u32且回包不回显goods_type_id，因此不虚构键控缓存。62202强化、62203穿戴、62204脱下继续DEFER，62205/06无当前wire不复活；不接UI、配置、背包/属性、事件、红点或本地派生。
- Unity ProtocolCoverage：`registered=1102`，`liveDefined=1468`，`liveGap=488`，未注册族错误出口10个；运行时真实注册覆盖 `980/1468=66.8%`，A～E 治理断言全通过。
