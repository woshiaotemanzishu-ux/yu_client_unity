...
- 最新完成轮次：R480，接入 Partner 14200 单向场景伙伴外观通知：精确解析 `type_id:u8,role_id:u64,figure_id:u32`，独立保存不可变最近原始包并发专用事件，不污染同修面板数据；伙伴场景模型消费 API 尚未迁入，故不臆造角色缓存/渲染。14203 出战牵涉场景与 21002 技能刷新，14206 是物品消耗培养，14207 是传记解锁奖励，三项继续按完整产品链 DEFER。
