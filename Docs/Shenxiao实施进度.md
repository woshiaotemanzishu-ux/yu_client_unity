...
- 最新完成轮次：R561。B段现要求冻结`liveCoveragePercent`严格等于`totalLiveDefined/totalLiveGap`按一位小数推导的历史活覆盖率，防止顶层总量一致但百分比字段单独漂移。
- 正式历史总量表示`(1468-697)/1468=771/1468=52.5%`。只把百分比改成52.6的反向探针被B精确报为`历史覆盖率52.6!=总量推导52.5`，总判决失败。
- 恢复后的`coverage_20260802_192317.md`为`registered=1256/liveDefined=1468/liveGap=331/errorExit=20/active=1137/1468=77.5%`，A～H全PASS、`EXIT 0`，B显示家族冻结计数/逐号清单、历史活缺口分项/总量697与覆盖率52.5%均一致。正式baseline未改。
