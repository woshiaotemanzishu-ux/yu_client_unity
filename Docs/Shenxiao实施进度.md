...
- 最新完成轮次：R570。三路复核Equip遗留15218/15221/15222并全部纳入hard-negative：15218会校验穿戴装备/等级配置、真实扣料、事务写神装等级、重算属性，异常还会邮件返材；已有`Godequipbuild`转换Prefab尚无业务View，且缺页面所需完整配置/材料链。
- 15221会扣货币和具体背包实例、写套装DB、重算属性并推15000；15222会降级套装、写DB、重算属性并真实返物/邮件，换装不兼容时还可能由服务端主动下发。共鸣整页没有可编辑业务Prefab，必须等配置、材料/资产冻结、确认、单飞、错误、背包/邮件/属性和权威刷新整链闭环，禁止孤立sender或raw结果。
- 负向探针临时加入三号常量，F精确列出`15218/15221/15222[ProtoConst]`并令总判决FAIL；撤回后`EquipReadCase pass=True/restored=True/EXIT 0`，`coverage_20260802_204413.md`为`registered=1256/liveDefined=1468/liveGap=331/errorExit=20/active=1137/1468=77.5%`、A～M全PASS，治理分类更新为`80+24+227=331`、legacy仍为2族/4号。baseline历史`24/9/status`未追写，仅补152族人工状态说明。
