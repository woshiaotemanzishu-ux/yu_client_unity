...
- 最新完成轮次：R562。A段现要求冻结顶层总量能严格分解为`2371=活覆盖771+活缺口697+非活已注册24+死缺口879`，并由893个历史注册号推出98个手写号；`denominatorNote`必须由这些值通过统一函数精确生成。
- 两枚反向探针分别覆盖结构与文案：`deadGap=904`被A报为非法分区；仍数学合法的878被报为`denominatorNote与冻结总量不一致`。报告生成与基线核验现复用同一口径函数。
- 恢复后的`coverage_20260802_193328.md`为`registered=1256/liveDefined=1468/liveGap=331/errorExit=20/active=1137/1468=77.5%`，A～H全PASS、`EXIT 0`，A详情打印完整历史分区与手写98并确认口径文案一致。正式baseline未改。
