...
**镜像定案改判(2026-06-12,v3)**:mirrorX=true 才正确。原因:Laya 的 UI 模型相机
rotY180 → 屏幕x=世界-X;Unity 相机屏幕x=+X。同一份几何两边显示互为镜像,必须在
转换时做 YZ 平面镜像(位置 -x、四元数 (x,-y,-z,w)、三角形换序、法线/绑定姿势/动画曲线
同步)才与老客户端同向。铁证:武器全职业右手单持(WeaponCountInfo),老客户端剑在
画面左侧(=角色右手),mirrorX=false 时剑在画面右侧。此前「mirrorX=false 即正确」
是在近似对称模型上的误判。
