...
## Web 启动、选角与首场景分段加载（2026-08-04）

- Web 现在有两段连续且职责明确的加载反馈：`WebGLTemplates/Shenxiao/index.html` 在 Unity Player 可用前显示“连接资源服务器/下载游戏引擎/启动游戏”及百分比、预计时间；Unity 接管后由唯一的 `LoadingManager → LoadingView` 显示协议、资源、地图和实体阶段。两段不能同时存在，也不能在中间露出无文案黑屏。
- 登录初始化只阻塞登录舞台与账号页，其余登录页后台加载。角色选择预加载只包含小型 UI、职业图标/说明、角色头像及默认选中角色的首屏模型；不得再阻塞全部职业视频、全部职业模型和账号下全部角色模型。2026-08-04 Web 实包由原 `83项/279.5MB` 降为 `34项/31.7MB`，其余资源在玩家选择时按需加载。
- `GAME_START` 首屏闭包只包含主 UI/配置、当前角色 `idle/run`、当前地图首屏和必要常驻效果。全职业战斗动作及公共资源预取必须等场景揭幕后后台执行，禁止与地图和主角首帧争抢 WebGL 主线程。
- 进度区间固定为：启动协议 `3%~25%`、游戏资源 `25%~80%`、场景资源/解析/构建 `80%~95%`、实体 `95%~100%`。下载阶段按实测速率估算；“绘制首屏地图”使用约 5 秒阶段预算，“生成角色与怪物”使用约 2 秒预算，不能拿前一段网络下载速率误报“不到 1 秒”。
- 首次进世界且 `LoadingManager` 可见时，`SceneController` 不再叠加更高层的纯黑 `SceneTransitionMask`；加载页保持到地图首帧和实体就绪后一次性揭幕。普通换地图仍保留原黑幕过渡。
- 浏览器证据与逐帧样本位于 `output/ui_route_audit/2026-08-04_web-foundation/evidence/`；连续采样未出现“加载页→黑屏→加载页”的回跳。

## 游戏内切换账号/角色的登录 UI 恢复

- 场景首屏就绪后，`LoginFlow.ReleaseLoginViews()` 会释放登录舞台及各页面，以归还大图和 bundle；因此游戏内退出流程不得直接调用只操作现存引用的 `ShowLogin()`。
- “切换账号”确认后必须先关闭游戏内自动重连、主动断开并让各业务模块清场，再通过 `LoginFlow.RestoreAfterLogoutAsync(true)` 重建整套登录 UI，最后显示账号登录页。
- “切换角色”确认后执行同样清场，但通过 `RestoreAfterLogoutAsync(false)` 保持登录页隐藏并显示连接等待层；随后复用当前服务器入口重连并发送 `10000`，由角色列表回包进入真实选角页。
- 登录 UI 释放时必须同时注销角色列表、断线、进游戏、`GAME_START`、场景首屏事件和预加载进度订阅；重建时再统一注册，禁止跨会话残留重复回调。
- 重建或重连失败必须给出可见提示；角色重连失败回退到账号登录页，禁止留下空 Window 层形成黑屏。
...
- `LoginStage` 保留原有的 `Awake/OnValidate` 比例同步：它只读取 Prefab 当前 `Source Image` 的原始尺寸并更新同节点 `AspectRatioFitter.aspectRatio`，不加载、不查找、也不替换任何背景资源。`LoginFlow` 同样不设置背景图片。
