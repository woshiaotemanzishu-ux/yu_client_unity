...
- 协议覆盖 baseline 收口规则：`Schemas/ProtocolCoverage/baseline.json` 的历史 `unityRegistered/liveGap` 数值用于防倒退，禁止为追当前报告逐轮改写；`registeredCmds`必须与`totalUnityRegistered`等长、全部为五位协议号、唯一且严格升序，每个`families[].unityRegistered`又必须精确等于该逐号清单按`cmd/100`分组后的数量，禁止删空、损坏逐号清单或下调家族计数来弱化A/B。但当最新运行时扫描的某家族全部活缺口已由注册或带 evidence 的 killlist 完整治理时，必须在同轮把人工 `status` 转为 `done` 并写 `statusNote`。`ProtocolCoverageCase` 的A段负责历史注册号逐号防退，B段负责家族冻结计数与逐号清单一致及当前家族不倒退，C段负责阻止已完工族重开未治理缺口，H段反向阻止零活缺口或全量 evidence-killlist 家族继续滞留 `pending/legacy_unverified`；若`legacy_unverified`族的当前活缺口已全部进入有效killlist/hard-negative且至少一项仍为hard-negative，则已完成分类但未完成业务，必须人工转`pending`而非`done`。正式baseline的家族前缀必须唯一，任何仍有当前活缺口的`done`家族都必须保留非空`statusNote`。禁止借状态逃逸门禁，也禁止把真实玩家可达写事务塞进killlist来伪收口。候选 `baseline.next.json` 必须保留正式基线的人工 `status/statusNote`，另以 `suggestedStatus` 输出本次机器建议；不得用候选覆盖人工策展状态或冻结历史计数。

- 协议 baseline 活缺口汇总规则：正式`baseline.json`的每个`families[].liveGap`都必须非负，179族分项总和必须精确等于顶层`totalLiveGap`，顶层值必须位于`0..totalLiveDefined`；`liveCoveragePercent`必须精确等于两项总量按一位小数推导的历史活覆盖率。顶层总量还必须能分解为活覆盖、活缺口、非活已注册、死缺口与手写号，`denominatorNote`必须由这组冻结值通过统一函数精确生成。A/B段必须在当前回归比较前校验这些冻结事实。禁止单改家族分项、顶层汇总、百分比或口径文案制造自相矛盾的历史基线，也禁止为追当前扫描结果逐轮重写这些历史值。
...
- 协议覆盖报告当前值规则：`coverage_*.md`总量表必须只写同次扫描当前值，历史值只能进入明确标注的baseline对比；族错误出口行必须显示当前未注册数并另列当前/baseline差值，禁止把历史34伪装成本次20。断言标题必须按实际`AssertionOutcome`动态覆盖A到最后一段，正文逐条无遗漏。`ProtocolCoverageCase`的J段必须在写报告前校验抬头、七项当前总量、错误出口对比、断言范围与正文，并对加入J后的最终文本再自检。
...
- 协议 killlist 运行时零交集规则：`Schemas/ProtocolCoverage/killlist.json` 必须非空，每条都必须是五位协议号并带非空 `reason/evidence`，且不得拥有任何运行时 S2C handler；`ProtocolCoverageCase` 的 G 段必须独立校验字段完整性、清单唯一性与 `NetManager._handlers` 零交集，禁止依赖C/H的间接失败掩盖坏数据。每条默认 `clientMode=absent`，不得残留 `Proto` 常量或生产发送引用；只有双端证明C2S入口真实可达、S2C专属回包死亡的半死号才可显式写`clientMode=send_only`，此时必须同时保留发送常量和生产目录中去字符串/注释后的直接`Send*(Proto.X,...)`调用，但仍禁止注册handler。经R551复核，当前唯一七个send-only例外为11101、30008、40029、40218、40505、40507、62107；局部变量、五位协议号数字字面量、注释、字符串、测试或常量的非发送引用均不算发送证据，任何生产`Send*(12345,...)`数字直发都必须被F/G门禁审计，禁止绕过`Proto`常量。产品或双端事实推翻旧死亡裁决时，必须同轮从killlist删除该号并恢复完整消费者，禁止让“已复活号”继续伪装为死号。冻结 baseline 中已注册、后经证据裁决移除的号由 A/B 段按具体编号补偿，禁止改写历史计数掩盖删除；R548已清退13802、19007、33225、33226四个防御性空handler，并从过期killlist移除Baby 18208/09/16/17/18/19/20/24、19501与40230。
...
- R519/R569 Goods 15011/15014/15015/15023/15028：15014/15015战力分项仍只允许从他人资料卡显式触发，按`server_id+role_id`完整替换raw快照；缺`CLIENTFIGHTCOM`时只按wire索引并排显示，禁止猜名称、分组、求和或加入GAME_START。15023维持老端无触发且服务端恒失败KILL。15011旧资料查询与现役19502都从同一`GoodsStatus/lib_goods_util`源构造相同九字段装备元组，而19501→19502已统一同服/跨服入口并额外携带法阵/仙灵，Unity唯一`LookOverFlow`、模型、页面和用例均消费19502；15011按现产品链已替代KILL，禁止恢复常量、sender、handler或第二份资料快照。15028会真实扣料并产出物品，继续保留活缺口并进入hard-negative；在`CompositeModel/config_compound`、材料选择与冻结、背包权威校验、单飞、成功/失败结果和奖励展示完整迁移前，禁止新增常量、sender、handler、raw ACK或让当前降级按钮发包。

- R520 Recharge 15802/15803：15802是S2C-only严格空体充值成功通知，每个包都必须标记已收到、显示一次“充值成功”并发一次无参事件；不得自动重查或推导资产。15803只允许显式严格空查询，也接受充值入账后的服务端主动推送；S2C `total_gold:u32` 是绝对累计钻石数，每包含0都完整替换独立loaded切片，请求无回复保留旧值。GAME_START与跨天仍严格只走 `45000→45004→15800`，禁止加入15803或15901；15804会扣直购币并发礼包，继续等待支付确认、配置、资产与结果UI闭环，禁止裸sender或乐观成功。

- R521 LoginNotice 10207：本号是S2C-only `type:u8`，无请求；每包保留raw type，只有非0才串行重查 `url_cdn_path/login_notice/jzy/login_notice.json.cfg.v`，版本变化或内存无正文时再取 `.json.cfg`，正文解析失败必须保留旧快照且不得先提交版本。公告只按 `plat_belong` 的wire ID顺序、`plat_name` source前缀和服务端时间窗筛选，`open_login/open_inside` 分流；登录自动弹按 `new_reg/show_rule(1版本、2每日首次、3每次、4不弹)`，游戏内红点按角色ID隔离并执行 `red_dot_rule(1/2新增或内容变更、3每次选角登录、4每日首次且未读续存)`。标题项只允许根透明Image命中并用真实 `GraphicRaycaster→PointerClick` 验收。当前417外层福利页未迁移，入口只受控直达已完成的游戏公告子页，红点与既有福利聚合；不得伪造C2S、加入GAME_START、从资源CDN取公告、跨角色共享已读、解析失败清旧或把10211弹窗链混入。
...
- R540 NineSky 13507纠偏：R126原规则已明确不接13505-13510，R528晚增的13507常量、注册、结算raw模型与正向用例现已全部撤销；服务端结算推送存在不构成绕过旧边界的授权。13507继续缺完整结算UI、场景退出与奖励消费链，保持DEFER且不得注册no-op/raw占位。13502参战、13505离场和13509连杀同样继续DEFER。

- R529 NineSky 13508：本号保持KILL。服务端`score_ref`和`after_kill_player`仍正常累计积分/发服务端奖励，13503也会返回权威总分，但两处`pt_135:write(13508,...)`及其`send_to_uid`均整组注释；全仓没有其它直接或动态Cmd发送点，只有pt writer定义和老端孤立接收器。禁止新增13508常量、注册、raw切片、积分Toast、事件或对13503分数的补丁；只有服务端恢复活跃writer且产品明确恢复即时积分提示时才重审。13502/13505/13507/13509继续DEFER。
...
- R507/R523/R526 Designation：41104是S2C-only激活通知，41105场景通知、41107显式战力查询和41108移除通知继续只保存彼此隔离的最后raw切片，均不得补丁式改写41101；不得复刻旧端41104后自动发送41102。41109道具激活与41106升阶现均只允许从真实Dsgt详情页显式点击，合用10秒单飞且互斥；发送前必须具备41101权威状态、对应配置和真实BagModel足量。41106 C2S固定`id:u32`，只有已激活的`main_type=3`称号可用，必须同时存在当前阶与下一阶`config_dsgt_order`，实际材料严格取当前阶`consume`且恰好一条type=0背包物品；S2C固定`errcode:u32,order:u8,power:u32,currentused:u32,dsgtid:u32`。41106/41109每包分别完整替换独立raw结果并解除单飞；失败保留列表，只有`errcode==1`才提示成功并精确空发一次41101重查，重查前同一称号保持刷新门禁。禁止本地扣物、乐观激活/升阶/佩戴、用结果order/currentused补丁列表或加入GAME_START。41102/41103仍因场景称号表现缺失DEFER，41110继续DEFER。
...
- R512/R543 AutoBrush 13309/13310/13323/13324：GAME_START 必须先 Reset，再严格空发 `13300→13301→13309→13323→13324`，不得自动发送13310。13309 为 `code:u32,next_stage_reward_gate:u64` 原始绝对快照，禁止把 gate=0 在模型层改成旧 UI 的99999；13323为`node:u8`；13324为`daily_ask_time:u16,next_ask_time:u32`并接受同号主动推送。13310只允许用户点击现有`AutoBrushModule.prefab`的`_box_show/_box_click`显式触发，发送前必须已有`13309 code==1`、gate非0且不超`long.MaxValue`、当前关卡达到gate；同一事务10秒单飞，成功后在13309回包前继续锁定。13310回包`code:u32 + u16×{style:u8,type_id:u32,count:u32}`逐包完整替换独立raw结果，保序保重、零/最大值有效；失败保留13309且不回查，成功只展示真实奖励并精确空发一次13309，不得本地推进gate、扣/加背包或把展示列表当二次发奖。13322教程节点持久化继续DEFER；不得附带教程UI、公会协助操作或乐观状态。
- R513 WeekDungeon 50805：仅接S2C周本专属结算，wire固定为 `result_type:u8,dun_id:u32,go_time:u32,dun_rewards:u16×{type:u8,times:u16,ObjectList},role_boss_list:u16×{boss_id:u32,reward_st:u8,ObjectList}`。每包完整替换独立loaded快照，保序保重、空表清旧、零/最大值有效；不得暴露请求、复用61003、改50801/50802、接结算UI/场景退出、展示或发放奖励、补查、配置、事件或背包状态。R513时延期的14402现已由R522完整闭环；13213挂机赎回仍继续DEFER。
...
- R541 Game 10205纠偏：KfStage原规则在R111已明确“不接10204/10205/10208/10209”，R484晚增的10205常量、常驻注册、Toast/日志handler与正向用例现已撤销。`lib_game:send_error*`活跃且老端有全局消费者仍不足以绕过原产品边界；10205保持真实活缺口、不得注册no-op或另建raw模型。10204继续`old_client_unreachable` KILL；R521的10207完整公告链不受影响；10211仍随`data_popup`、登录/定时/Temple条件和真实弹窗消费者延期。

- R541/R545 BrightSea/SnatchTreasure纠偏：R179/R167-R179已明确不接18903/18905-18914与18918-18920，R170已明确65208归ActivityForeshow且不接65200/65202-08；晚增的18905、18918/18920与65206常量、注册、handler、模型、sender和正向用例现已全部撤销。BrightSea只保留18900/01/02/04/15/16/17；SnatchTreasure只保留65201，65208仍由ActivityForeshow独占。四号即使有服务端结果、错误推送或权威退出链也继续作为真实活缺口pending，不进killlist，不得以“只接回包/统一错误/服务端切场”为由恢复。
...
- R540 Eternity纠偏：R182晚增的27906以及同批27904/27905/27907/27908/27909均晚于R129的`Do not attach 27902-27909`且未改写原禁令，现全部撤销。永恒圣殿只保留27900时程与27901参与资格；27902-27909继续整体不接。
...
- R516/R570 Equip 15217/15219/15220/15223/15262：装备家族 GAME_START 内部顺序必须保持 `15214→15217→15220→15210(pos=1..10)→15261`；15217神装和15220共鸣位置表均为原序全量快照，保留重复项，空表loaded清旧。15219回包不回显pos，只保存最后一份`power:u32`原始试算；15223按回包`(equip_type,make_type)`、15262按回包`(pos,type,lv)`键控完整替换，奖励/战力表保序保重、空表有效，15262的`combat`保留u64全位。请求无回复不得清旧，各切片互不交叉覆盖。15202维持`old_client_unreachable` KILL。15218会校验穿戴装备/配置并真实扣料、事务写神装等级、重算属性，异常还会邮件返材；15221会扣货币和具体背包实例、写套装DB、重算属性并推15000；15222会降级套装、写DB、重算属性和真实返物/邮件，且换装不兼容时可由服务端主动下发。三号进入hard-negative：只有对应配置、可编辑业务页、材料/资产冻结、确认与单飞、错误、背包/货币/邮件、属性和权威结果刷新全链闭环时才成组重审；禁止孤立常量、sender、raw handler、Toast、乐观扣物或局部模型补丁。
...
- R575 15403/15902/19103/33804/61201资产交易事务边界：15403按拍卖类型、商品、竞价方式和绑定/非绑货币跨进程竞价，成功真实扣款并依赖拍卖广播、退款/邮件和权威状态；15902会校验福利卡购买/当日领取/背包，扣剩余天数、写DB并按双倍周经背包或邮件发奖；19103会校验礼包激活、期限、档位和购买上限，真实扣费、发奖、写购买次数并改活动内存；33804会校验竞榜活动、未领取与当日积分，按世界等级发奖并写领取DB/内存；61201会自动购买成本、发奖、写档位已购状态，也会在充值到账路径主动推送成功。五号全部进入hard-negative：现有15401/02/07-11、15901、19101/02、33800-03和61200/03读侧不授权资产操作。只有对应可编辑业务页、权威配置/资格与完整状态、成本/资产冻结、确认与单飞、错误、奖励/背包/钱包/拍卖/活动状态刷新、重复操作防护及61201主动推送去重全部闭环时才逐号重审；禁止孤立常量、sender、raw ACK、成功Toast、局部领态/购买次数补丁、乐观扣款或本地发奖。

- R576 VIP 45001/45002/45003/45007/45008写事务边界：45001按VIP等级校验、写已领列表，可能扣配置成本并发等级奖励，且当前服务端先落领取DB再校验成本；45002写周礼包领取态并真实发奖；45003扣勾玉、启用特权卡，事务写卡/VIP等级经验/周状态并派发购买事件与首次权益；45007按免费资格启用卡片，事务写卡/VIP等级经验/周状态/免费领取态并派发事件；45008写`vip_hide` DB和角色figure，更新场景代理、广播12010并写角色展示。五号全部进入hard-negative：45000/04/05/06及15802/03只读/通知切片不授权写操作。只有完整可编辑VIP/特权卡/设置页、权威配置与资格、钱包/成本/背包指纹、确认与单飞、错误、奖励/称号/卡片/等级经验/周状态/场景外观刷新、重复领取防护及45001异常写入顺序兼容全部闭环时才逐号重审；禁止孤立常量、sender、raw ACK、成功Toast、乐观扣费/领态/隐藏或本地发奖。

- R577 10211/13213/13322/17501/17503编排与状态事务边界：10211既是严格空请求，也是服务端按`data_popup`在登录、周期定时和Temple完成路径主动推送`uid:u32`的功能弹窗编排，依赖定时器生命周期、条件筛选与真实功能消费者；13213按次数赎回离线经验，可能自动购买并消耗离线物品，更新次数DB和角色经验；13322把教程节点写入`enchantment_guard`并持久化，老端发送时还会乐观`SetStep`；17501/17503分别校验普通/合服七日状态，经背包或邮件真实发奖并写领取态。五号全部进入hard-negative：现有10207公告、13200-12挂机、13300/01/09/23/24及17500/02只读地基不授权这些编排或写事务。只有10211的配置、登录/定时/Temple触发、路由与弹窗消费者整链，以及其余协议对应可编辑页面、权威配置/资格/状态、消耗或次数冻结、确认与单飞、错误、奖励/经验/教程/图标红点/背包邮件刷新、重复操作防护全部闭环时才逐号重审；禁止孤立常量、sender、raw uid/ACK/奖励、成功Toast、乐观节点/领态、自动购买或本地发奖。

- R578 Designation 41102/41103佩戴事务边界：两号C2S均为`id:u32`；41102 S2C为`code:u32,id:u32`，41103只回`code:u32`。41102会校验不是当前称号、已激活且未过期，把旧称号写未使用、新称号写使用状态，更新角色figure与场景代理并广播41105；41103只允许卸下当前且仍存在的称号，写未使用状态、清figure、更新场景代理并广播41105。两号进入hard-negative：当前Dsgt详情页明确隐藏`dsgt_adorn_button`，`NameBoard`只实现血条，41105仍只保留隔离raw通知，缺主角/他人称号显示消费者。只有真实Prefab佩戴按钮、41101权威列表与配置/时效二次校验、互斥单飞、错误、成功后唯一41101刷新、主角及其他玩家RoleVo/NameBoard称号图文/特效、41105场景广播消费、切场景与遮蔽规则全部闭环时才成组恢复；禁止孤立常量、sender、raw结果、局部`CurrentUsedId`补丁、乐观佩戴/卸下或无场景表现的设置入口。既有41106/41109完整事务继续保留，不得因本轮治理回退。

- R579 MondaysAward 17901/17903/17906状态资产事务边界：17901 C2S/S2C均为`u16×{id:u16,rids:u16×u16}`嵌套奖池，服务端校验类型唯一、每类数量及可选奖励后替换角色`now_pool`，后续抽奖写DB；17903 C2S严格空，S2C为`type:u8,pool_id:u16,draw_times:u16`，会按当前池随机、真实扣配置成本、发奖、累计次数、写个人记录与角色DB、推17902并进入跨服记录/传闻/成长福利链；17906 C2S为`task_id:u16`、S2C为`code:u32`，会校验已完成未领、真实发奖、写任务已领状态与DB并主动推17904。三号进入hard-negative：现有17900/02/04/05/07/08独立raw地基不授权选池、抽奖或领奖。只有完整可编辑奖池/抽奖/任务页、权威四份配置与活动时段、17908池和17907次数二次校验、抽奖券/背包指纹、确认或节流与互斥单飞、17900错误、随机结果动画/奖励/背包/记录/次数/任务状态刷新及重复领取防护全部闭环时才逐号恢复；禁止孤立常量、sender、raw成功结果、乐观换池/扣券/累计次数/任务领态或本地发奖。

- R580 Medal 13402/13403/13404/13406/13407写事务边界：13402 C2S严格空、S2C为`id:u32,honour:u64`，会扣勋章配置成本、写等级DB、改figure/scene、重算属性并推进任务、成就、礼包、Temple、活动和至尊VIP；13403 C2S为`id:u32`、S2C为`code:u32,id:u32,level:u16,power:u32,is_equip:u8`，会扣物激活/升星、写DB和属性，首次激活还自动佩戴并同步/广播场景；13404 C2S为`id:u32`、S2C为`id:u32,code:u32`，会切换新旧头衔DB佩戴态及role/figure/scene；13406 C2S严格空、S2C为`code:u32`，会清佩戴态和figure并同步场景；13407 C2S为`u16×{goods_auto_id:u64,num:u32}`、S2C为`lv:u32,exp:u32,power:u32`，会删除具体背包实例、写强化等级经验、重算属性并同步场景战斗属性。五号进入hard-negative：现有13400/01/05三份raw地基不授权操作。只有完整可编辑勋章/头衔/强化页、权威配置/等级/状态、背包实例与资产指纹、材料选择、确认与互斥单飞、13400错误、成功后13401/13405唯一权威重查、奖励/背包/属性/任务/活动/主角与他人场景头衔刷新及首次激活自动佩戴兼容全部闭环时才逐号恢复；禁止孤立常量、sender、raw结果、成功Toast、乐观晋升/佩戴/强化、本地删物或属性推演。

- R581 Partner 14203/14206/14207场景培养奖励边界：14203 C2S为`companion_id:u32`、S2C为`code:u32,fight_id:u32,figure_id:u32`，成功会持久化唯一出战伙伴、重算伙伴/角色属性、更新场景训练对象、广播14200并由`pp_skill`触发21002技能刷新；14206 C2S为`companion_id:u32`、S2C为`errcode:u32,companion_id:u32`，会按配置真实耗尽本次可用培养物、事务写培养次数、重算属性、通知角色属性并推14201；14207 C2S为`companion_id:u32,biog_lv:u8`、S2C为`code:u32,companion_id:u32,biog_lv:u8`，会持久化解锁、重算属性并真实发奖，成功时先推14202权威全量再回14207。三号进入hard-negative：现有14200场景通知及14201/02/04/05伙伴raw不授权操作，且Unity尚无14200场景伙伴消费者，21002已由`SkillController`独占注册。只有完整可编辑伙伴/元神/传记页、权威伙伴与传记/培养/奖励配置、背包与资产指纹、主角和他人场景伙伴实体、技能刷新唯一所有权、确认/节流与互斥单飞、错误、14201/14202权威重查、属性/背包/奖励展示及重复解锁防护全部闭环时才逐号恢复；禁止孤立常量、sender、raw ACK、重复注册21002、成功Toast、乐观跟随/培养/解锁、乐观扣物或本地发奖。

- R582 WX 11301/11302/11306产品边界：11301 C2S严格空、S2C为`count:u8`，服务器只读当日分享次数，但旧端把它加入FriendInvite GAME_START、等级补拉和跨天并依赖`config_wx_kv`更新分享剩余次数；Unity已锁定`34001→34012(3)→34005→34006→34008(60)`生命周期，缺微信分享配置、SDK和消费者。11302 C2S严格空、S2C为`error_code:u32`，会校验日次数、写`role_wx_share`、真实发奖并先推11301；旧端由SDK分享后的0/3秒超时触发并弹奖励。11306 C2S为`temp_list:u16×temp_id:u32`、S2C为`temp_list:u16×{temp_id:u32,state:u8}`，依赖`config_wx_subscribe_temp`构造模板集合，服务端异步查询并按请求逆序返回，11305订阅成功还会主动推单模板状态。三号进入hard-negative并继续保持现有11307独立总开关；只有微信/QQ分享SDK回调、两份权威配置、分享入口与次数消费者、超时/失败/防重、背包和奖励表现，以及模板订阅SDK、11305/11306状态机、设置页与主动推送消费全部闭环时才逐号恢复。禁止改FriendInvite启动/等级序列、加入跨天请求、孤立常量/sender/raw切片、模拟SDK成功、乐观次数/模板状态或本地发奖。
...
- R586 NineSky 13502/13505/13509战场生命周期边界：三号继续受R126“只接13500、不得接13501-13510”及R540纠偏后的现行边界约束并进入hard-negative。13502收发严格空请求/`error_code:u32`结果；成功会建立或恢复九魂排名与房间状态、可能真实发首登/层数奖励、推13502/03/04、切入本服或跨服场景、加动作锁并触发活动日历/参与事件。13505同样严格空请求并回`error_code:u32`；在九魂场景内会修改在线态/房间人数，持宝者还清旗、重建秘宝、广播13504，再强制切回普通场景。13509为S2C-only `role_id:u64,Figure,dkill:u16`全场连杀表现包；旧端本人`dkill>2`逐次展示、他人每5杀展示，排队消费完整Figure头像/姓名/次数和动效。13507沿用R540 hard-negative，13508保持R529 KILL；13506/13510等死链不因本轮复活。只有完整可编辑九魂入口/战场/复活/结算页、权威配置与奖励、场景切换/动作锁/自动战斗、房间/秘宝/其他玩家完整Figure、击杀队列与动效、13503/04状态消费、错误/确认/互斥和重复奖励防护全部闭环时才逐号恢复；禁止孤立常量/sender/raw ACK、no-op 13509、乐观入退场、只关UI、猜场景或本地发奖。

- R587 Eternity 27902/27903场景生命周期边界：两号继续受R129“只接27900、不得接27901-27909”及R540纠偏后的现行边界约束并进入hard-negative。27902 C2S为`scene:u16`、S2C意图为`code:u32`；旧端主面板、Boss归属提示和场景内跳转均有真实入口，服务端会校验开放/人数/归属，写本地与跨服参与表，触发任务、至尊VIP和活动日历，随后把角色切入永恒圣殿、设置服务器PK及场景HP；成功ACK晚于真实场景操作且不能替代场景权威。27903 C2S严格空，退出确认后服务端会切回默认场景、恢复和平PK、清本地/跨服参与表；但当前实现错误调用`pt_284:write(27903,...)`，该模块无27903子句而落`cmd=0`空包，客户端不得补偿为27903成功。27904-27909沿用R540 hard-negative，至此279族八个当前活缺口全部机器治理；27900时间快照与27901显式参与资格保持唯一运行时注册。只有完整可编辑永恒圣殿入口/场景/Boss/伤害/复活页、权威配置和开放/人数/归属规则、跨服场景与PK/HP/重连/退出生命周期、27904-27909怪物/伤害/死亡/墓碑/错误消费，以及服务端修正并实证27903正确回包后才逐号恢复；禁止孤立常量/sender/raw ACK、乐观入退场、只关UI、客户端伪造27903成功或猜测场景状态。

- R588 Rune 16703/16707/16708/16710/16711完整业务边界：五号继续受R505“不得接兑换、觉醒、拆解、技能升级、卸下”的现行约束并进入hard-negative。16703 C2S为`id:u16,num:u32`，S2C为`code:u32,rune_chip:u32,id:u16,num:u32`；服务端按配置与数量扣符文碎片、写Rune DB，并经背包或邮件真实发物。16707 C2S为`goods_id:u64,attr_id:u16,cost_ids:u16×u64`，成功回完整等级/觉醒/经验/战力；服务端会删除选定材料实例、写目标觉醒数据和符文等级，更新被动技能、角色属性、场景、战力及排行；当前两个失败分支只传6项给要求8项的`pt_167:write(16707,...)`，不得由客户端猜字段或伪成功。16708 C2S为`goods_ids:u16×u64`，S2C为`code:u32,goods_results:u16×{goods_id:u64,ObjectList}`；会清单属性觉醒数据或删除多属性符文实例，并经背包/邮件返还材料。16710严格空请求，回`code:u32,lv:u32`，会持久化技能等级、更新被动技能/场景/属性/排行。16711 C2S为`pos_id:u8,goods_id:u64`，S2C为`code:u32,pos_id:u8,goods_id:u64`；会把穿戴符文移回符文背包、重算属性/被动技能/场景/排行，旧端还在合成流程中先批量卸下再等待背包变化续发合成。现有16700/01/02/04/05/06/09保持不变。只有完整可编辑兑换/觉醒/拆解/技能/卸下与合成页、权威配置、背包实例/符文碎片/觉醒材料指纹、确认/互斥单飞、16700权威刷新、物品/邮件/属性/技能/场景/排行/动效消费，以及服务端修正并实证16707失败回包后才逐号恢复；禁止孤立常量/sender/raw结果、乐观扣料/碎片/等级/卸下、本地删物发奖、复制旧端脆弱合成等待或客户端补偿坏包。

- R589 SentientAct 24103/24104/24105/24108/24109场景与组队边界：五号继续受R164/R525“24103-05/08/09场景/操作链继续不接”的现行约束并进入hard-negative。24103 C2S为`portal_id:u64`、S2C为`code:u32`；只允许队长触发，服务端校验当前门户后把ID写入玩家状态，旧端紧接Team 24020启动玩法，后续组队入本条件再次验证并把该门户传入副本角色，不能拆成孤立ACK。24104/24105均严格空请求并回`code:u32`：前者检查动作与重复入场后把角色调度到本服/跨服众生之门场景，重置`group/ghost`；后者只在该场景成功，切回默认场景并按100%血量恢复。24108严格空请求并回`code:u32`，发起者先走24104入场；队长还会向其余队员广播24108，旧端逐人弹确认，确认后各自再发24104，禁止no-op注册或自动同意。24109严格空请求，回`dun_id:u32`；它只在旧端队伍页打开时为装备聚合页从本服/跨服副本配置随机取一个展示ID，实际组队检查、匹配和入本会独立再次选取，故不是权威目标或可缓存的玩法状态。现有24101/02/06/07及GAME_START `24101→24107→24102`保持不变。只有完整可编辑众生之门入口/场景/队伍/装备聚合页、权威门户和副本配置、Team 24020建队/匹配/入本编排、跨服场景切换/重连/退出、逐队员确认与错误/互斥，以及副本实际ID消费者全部闭环时才逐号恢复；禁止孤立常量/sender/raw ACK、把24109当权威、自动查询/同意、乐观选门/入退场、只关UI或绕过Team链。

- R590 JJC 28005/28008/28012/28015/28017资产与战斗边界：28005/12/15/17继续受R506现行DEFER约束并进入hard-negative，28008按既有KILL裁决进入killlist。28005 C2S为`num:u16`、S2C为`errcode:u32`；服务端按当前daily购买量逐档汇总配置成本，真实扣资源、增加daily购买数和挑战次数，并写资源找回与消费日志，失败统一走28000。28012严格空请求并回`errcode:u32`；老端28003结果后若仍在竞技场场景会自动发送，服务端停止battle，退出回调清`jjc_battle_pid`、释放动作锁并回成功，terminate还清场景房间和排名战斗占用态。28015严格空请求并回`errcode:u32`；存活battle会立即结算挑战结果、设置`clear_status`、清除假人，另推28014阶段2绝对截止时刻。28017 C2S为`break_id:u32`，S2C为`errcode:u32,break_id:u32,ObjectList`；按历史最高排名、配置、已领表与背包容量校验，成功持久化`break_id_list`并真实发奖，旧端再重查28001和展示奖励。28008历史C2S严格空、S2C为`errcode:u32,is_reward:u8,ObjectList`，但老端全仓只有接收壳而无发送点，服务端唯一handle和唯一`get_reward`调度整段注释，残留邮件/writer实现不可达；保持无常量、无注册、无请求。现有28000/01/02/03/04/09/10/13/14及GAME_START `28004→28001`保持不变。只有完整可编辑JJC主页面/购买确认/突破奖励页、权威配置和钱包/背包指纹、确认/互斥单飞、竞技场场景/battle/假人/阶段/结果/动作锁/退出生命周期、28001/04权威刷新和奖励展示全部闭环时才逐号恢复；禁止孤立sender/raw ACK、no-op 28015、乐观扣费/加次数/退出/领取、本地判胜负或二次发奖。
...
- R179/R541 BrightSea 18917 is an explicit-only strict-empty request with an independent full ship-status snapshot `auto_id:u64,status:u8,reward_times:u8,total_reward_times:u8`; every reply, including all-zero, replaces all four raw values. Activity-closed no-response preserves the old slice. Keep GAME_START as all-state clear followed by only 18900；R541已撤销晚增18918/18920，do not attach the old red-dot/UI/event behavior or 18918-18920.
...
- R173-R176/R538 FriendInvite: 34005 help snapshot is `count:u16,reward_list:u16×{reward_id:u8,status:u8},pos_list:u16×{invitee_id:u64,pos:u8,name:string,lv:u16,career:u8,status:u8}` and 34006 is independent level-invite snapshot with the same pos item. 34012 welfare status is query-only at fixed type 3; 34008 boost snapshot is `lv:u16,total_count:u16,ObjectList(u16×{type:u8,type_id:u32,num:u32})` at fixed lv60. Preserve wire order/duplicates; empty is loaded and clears only its own slice. GAME_START exact order is `34001 -> 34012(3) -> 34005 -> 34006 -> 34008(60)`. A true level change only re-runs that sequence when sharing is open and the ordinary 340 icon is absent; a box icon does not count, and same-level/share-closed/ordinary-icon-present events send nothing. Do not attach 34002-04/07/09, 11301-02, UI, red dots, claims or configuration。34010/34011的唯一服务端网络handler整段注释且全仓无其它调用者；老端34010模型写入已注释，34011零页面触发且残留sender把u64错发成u16。两号保持KILL，禁止注册、请求、红包列表、孤立ACK、奖励或客户端修复；只有服务端恢复handler并由产品恢复完整红包链时才成对重审。

- R170/R541 SnatchTreasure 65201: explicit-only strict-empty entry snapshot, never GAME_START. Wire is `belong_list:u16*{dunid:u32,score:u16,guild_id:u64,guild_name:string},territory_score:u16,have_territory:u8`; tail scalars are outside the list. Replace atomically, retain duplicate DunId/GuildId and wire order, and empty list remains loaded. 65208 stays exclusively in ActivityForeshow；R541已撤销晚增65206，do not attach 65200/65202-08, scene, battle, rewards, UI, config, sorting, or red dots.
...
## MondaysAward 17902/17904/17905/17907/17908 (R131/R148/R149/R153/R532)

- 17904 is the only parameterless GAME_START task-state snapshot: `task_state:u16*{task_id:u16,state:u8}`. 17902 is an explicit-only empty personal-record query: `count:u16*{role_id:u64,role_name:string,type:u8,pool_id:u16,utime:u32,picture:string,picture_ver:u32,career:u16}`. 17905 is an independent on-demand cross-server record snapshot: `count:u16*{server_id:u32,server_num:u16,role_id:u64,role_name:string,type:u8,pool_id:u16,utime:u32,picture:string,picture_ver:u32,career:u16,turn:u16}`. 17908 is an independent on-demand current-pool snapshot: `pool_count:u16*{id:u16,rid_count:u16,rid:u16*rid_count}`. 17907 is an independent on-demand draw-window snapshot `code:u8,draw_times:u16`; code is raw (only 1 is open) and DrawTimes is an absolute cumulative value. Each replaces only its own state, preserving duplicate RoleIds/Rids; every empty list clears old entries but remains loaded, while a request without reply preserves the previous slice. Do not reproduce the old client’s first-17904 automatic 17907 request or attach 17901/17903/17906, draw/claim/pool-change operations, config, red dots, or UI.
...
- GAME_START 必须发送14401的 `stage:u8=0,type:u8=0` 基础请求。回包为 `stage_list:u16×{stage:u8,type_list:u16×{type:u8,status:u8,pos_list:u16×{gtype_id:u32,pos:u8,status:u8}}}` 全量树；每包替换并允许空包清旧，保存前按stage/type/pos升序，`gtype_id` 必须保留u32。14402打造、配置与页面消费已由R522闭环，具体写事务约束以R522规则为准。
...
- GAME_START必须先清空VIP全部切片，再严格空发 `45000 -> 45004 -> 15800`；跨天只重拉同一只读子序列但不得清旧值。15901福利卡与15803累计充值均继续显式按需，禁止借启动或跨天恢复旧端的15901/15803自动请求。
...
- 45005是S2C-only激活通知，45006是S2C-only超时通知，二者均只覆盖各自最后一份 `card_type:u8,is_temp_card:u8` 原始切片，不得直接patch 45004；45006每包随后严格空查一次45004。15802是S2C-only空充值成功通知，15803是独立绝对累计钻石快照；它们与四个450切片、15800/15801和15901互相隔离，不改Role/Figure或本地资产。不接45001/02奖励、45003购买、45007领免费卡、45008隐藏VIP或15804直购币交易，也不接配置、红点、计时器、背包货币或本地奖励。
...
## Designation 41101/41104/41105/41106/41107/41108/41109（轮115/R507，R523/R526续接）

- 老端在背包初始化完成后空发41101；本端沿用 Fashion 同类迁移经验，简化为 GAME_START 空发一次（请求自身无背包参数）。回包为 `current_used:u32,items:u16×{id:u32,order:u8,end_time:u32}` 完整快照，服务端已负责清过期与特殊称号过滤，本端只全量替换、空列表清旧。R507增加41104/41105/41107/41108独立原始读侧；R523放行真实Dsgt页的41109道具激活；R526迁入4772条`config_dsgt_order`并放行同一详情按钮的41106升阶，两条事务成功后都只以唯一41101重查收权威状态，细节以前述R507/R523/R526硬约束为准。不得与Medal 13405普通标题表混为一体；不得借此恢复41102/41103佩戴、41110过期取消、场景称号、GAME_START自动操作或乐观资产状态。
