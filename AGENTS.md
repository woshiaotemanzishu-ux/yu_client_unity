...
- R537 Dress 11205：这是未激活装扮一级预期战力的显式只读查询，C2S固定`dress_type:u8,dress_id:u32`，S2C固定回显同键并返回`active_power:u64`。只允许玩家装扮详情入口按需发送，不得加入GAME_START、11200回包、类型遍历或自动补查；按`dress_type+dress_id`复合键逐包完整替换独立不可变快照，不同键共存、零值有效，请求无回复保留旧值。11205不得改写11200已激活列表、RoleModel战力或本地配置派生，也不得附带UI、事件、红点、Toast或资源预览。11201激活、11202使用、11203卸下仍是真实状态/资产事务，11204仍是他人场景装扮广播，四号必须随各自完整消费者闭环另审。
