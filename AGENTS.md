...
- R519 Goods 15014/15015 战力分项对比：同服显式请求15014为 `role_id:u64`，跨服显式请求15015为 `server_id:u16,role_id:u64`，两条成功链统一回15014：`server_id:u16,role_id:u64,other_power:u16×u32,self_power:u16×u32`。只允许从他人资料卡显式触发；按回包服务器/角色复合键完整替换不可变raw快照，保序保重、空表loaded、请求无回复保留旧值。当前缺旧端 `CLIENTFIGHTCOM` 权威配置，UI只按wire索引并排显示双方数值，禁止猜名称、分组、求和或总战力，也不得加入GAME_START。15023只有老端无触发的事件绑定与空handler，服务端检查又恒返FAIL，保持KILL；15011和15028继续待完整业务闭环。
