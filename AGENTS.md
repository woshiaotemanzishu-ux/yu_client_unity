...
- 协议 killlist 运行时零交集规则：`Schemas/ProtocolCoverage/killlist.json` 中每个协议号都必须带 evidence，且不得拥有任何运行时 S2C handler；`ProtocolCoverageCase` 的 G 段必须校验清单唯一性与 `NetManager._handlers` 零交集。每条默认 `clientMode=absent`，不得残留 `Proto` 常量；只有双端证明C2S入口真实可达、S2C专属回包死亡的半死号才可显式写`clientMode=send_only`，此时必须保留发送常量但仍禁止注册handler。R549当前唯一七个send-only例外为11101、30008、40029、40218、40505、40507、62107，禁止把字段当作任意死号保留常量的通行证。产品或双端事实推翻旧死亡裁决时，必须同轮从killlist删除该号并恢复完整消费者，禁止让“已复活号”继续伪装为死号。冻结 baseline 中已注册、后经证据裁决移除的号由 A/B 段按具体编号补偿，禁止改写历史计数掩盖删除；R548已清退13802、19007、33225、33226四个防御性空handler，并从过期killlist移除Baby 18208/09/16/17/18/19/20/24、19501与40230。
