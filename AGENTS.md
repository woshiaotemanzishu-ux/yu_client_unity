...
- R507/R523 Designation：41104 是 S2C-only 激活通知 `code:u32,id:u32,end_time:u32`，不得复刻旧端首次激活后自动发送41102；41105场景通知、41107显式战力查询和41108移除通知继续只保存彼此隔离的最后原始切片，不补丁式改写41101。41109道具激活现已完整闭环：只能从真实Dsgt详情页显式点击，C2S固定 `id:u32`，发送前必须同时具备41101权威未激活状态、`config_dsgt.goods_consume`恰好一条type=0背包物品及真实BagModel足量；请求10秒单飞。S2C固定 `errcode:u32,power:u32,currentused:u32,dsgtid:u32`，每包完整替换独立raw结果并解除事务单飞；失败保留列表，只有`errcode==1`才提示成功并精确空发一次41101重查，重查回包前同一称号保持“刷新中”且不得重复激活。禁止本地扣物、乐观激活/佩戴、用回包currentused补丁列表、加入GAME_START或从41104自动穿戴。41102/41103仍因场景称号表现缺失DEFER，41106仍因`config_dsgt_order`与升阶闭环缺失DEFER，41110继续DEFER。
...
## Designation 41101/41104/41105/41107/41108/41109（轮115/R507，R523续接）

- 老端在背包初始化完成后空发41101；本端沿用 Fashion 同类迁移经验，简化为 GAME_START 空发一次（请求自身无背包参数）。回包为 `current_used:u32,items:u16×{id:u32,order:u8,end_time:u32}` 完整快照，服务端已负责清过期与特殊称号过滤，本端只全量替换、空列表清旧。R507 增加41104/41105/41107/41108独立原始读侧；R523仅放行真实Dsgt页的41109道具激活，并以成功后唯一41101重查收权威状态，细节以前述R507/R523硬约束为准。不得与Medal 13405普通标题表混为一体；不得借41109恢复佩戴/升阶/过期取消、场景称号、GAME_START自动操作或乐观资产状态。
