...
- 节点：217（页面 30，叶 187）
- 叶状态：blocked=166，needs-runtime-verify=21，done=0
- 老端 TS 映射：48/48
...
| `marriage.main.mate.ask` | blocked | 权威婚姻态驱动的求婚按钮显隐尚未实现；当前只能静态打开目标页，不能把缺条件分支标为可运行验证。 |
| `marriage.main.mate.again` | blocked | 权威婚姻态显隐尚未实现，且再续入口没有传入伴侣身份参数；目标语义不完整。 |
| `marriage.main.mate.break` | blocked | 权威婚姻态显隐尚未实现，且解除入口没有传入伴侣身份参数；目标语义不完整。 |
| `marriage.main.mate.flow` | blocked | 权威婚姻态驱动的花房按钮显隐尚未实现；当前静态导航绑定不能替代条件分支实现。 |
...
| `marriage.main.dungeon.help` | blocked | 当前 Unity 仅日志占位，未打开对应说明页；缺真实目标路由、关闭链与运行态证据 |
...
| `marriage.ask.help` | blocked | 当前 Unity 仅日志占位，未打开对应说明页；缺真实目标路由、关闭链与运行态证据 |
...
| `marriage.flower.help` | blocked | 当前 Unity 仅日志占位，未打开对应说明页；缺真实目标路由、关闭链与运行态证据 |
...
| `marriage.main.lobby.query-17200` | needs-runtime-verify | 未启动 Unity/Web，缺真实发包、回包、父页即时消费与热重开证据 |
| `marriage.main.ring.query-17210` | needs-runtime-verify | 未启动 Unity/Web，缺真实发包、回包、父页即时消费与热重开证据 |
| `marriage.main.mate.query-17232` | needs-runtime-verify | 未启动 Unity/Web，缺真实发包、回包、父页即时消费与热重开证据 |
| `marriage.main.mate.query-17238` | needs-runtime-verify | 未启动 Unity/Web，缺真实发包、回包、父页即时消费与热重开证据 |
| `marriage.main.gift.query-17232` | needs-runtime-verify | 未启动 Unity/Web，缺真实发包、回包、父页即时消费与热重开证据 |
| `marriage.main.gift.query-17238` | needs-runtime-verify | 未启动 Unity/Web，缺真实发包、回包、父页即时消费与热重开证据 |
| `marriage.ask.query-17232` | needs-runtime-verify | 未启动 Unity/Web，缺真实发包、回包、父页即时消费与热重开证据 |
| `marriage.ask-list.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.ask.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.break.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.com.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.dsgt.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.flower.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.flow.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.gift-tips.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.honour.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.issue.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.record-com.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.record-flower.background-close` | blocked | 需在现有 Prefab 上增量接入背景点击面；本轮禁止用静态日志冒充点击关闭 |
| `marriage.dun-tips.gray-disabled` | blocked | 缺 DunTips 业务 View、主动方身份参数及禁用态射线拦截 |
| `marriage.legacy-boundaries.match-view` | blocked | 死路由；禁止仅因源码文件存在而补 Unity 入口 |
| `marriage.legacy-boundaries.match-tips-view` | blocked | 死路由；禁止臆造匹配提示入口 |
| `marriage.legacy-boundaries.tag-view` | blocked | 标签编辑由 IssueView 当前路径解释；旧独立窗保持不可达边界 |
| `marriage.legacy-boundaries.ring-change-view` | blocked | 替代路由属于 Common/Outward 且不在当前文件岛；禁止恢复旧 RingChange 写事务 |
