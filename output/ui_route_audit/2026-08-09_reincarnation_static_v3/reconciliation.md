...
- 主操作：`rebirth_btn`、`stageBtn`、`lock_img`、`_box_guard`、`keyAllBtn`、`goBtn`、`danBox`、`giftIcon`，以及四转后条件出现、解析 `@` 参数并调用 `OpenFun` 的 `tipsLabel` 富文本链接。
- 列表与滚动：任务列表/任务滚动、技能横向滚动、属性纵向滚动、装备/时装横向滚动；任务行有 `finishBtn`、`wayBtn`、`submitBtn`、`click_group`。
...
- 可点亮弹窗包含属性纵向滚动、材料/经验状态、`lightup_btn`、`shopBtn`、关闭；未解锁态隐藏点亮和商城按钮。
- 已点亮弹窗包含当前属性和累计点亮属性两块独立纵向滚动内容，以及点击背景关闭。
...
- 技能项：升级前/后图标均可打开 `ReincarnationSkillTipsView`；弹窗有前后描述两块纵向滚动区及点击背景关闭，需要核对目标身份和关闭链。
- 成功链：13040 后先进入 `ReincarnationEffectView`，旧代码目前因效果缺失直接回调打开 `ReincarnationSuccessView`；成功页包含属性列表/滚动、技能详情、头模/时装表现、转数图和动态效果，点击背景关闭。
...
- 已把源码可见的页面、页签、按钮、列表、滚动区、弹窗、条件块、协议、返回链和组件依赖冻结为独立 schema 6 manifest。v1 的 52 节点账因漏列 `tipsLabel` 条件链接而保留不改；v2 补到 53 节点；本目录 v3 继续补齐源码可见的各滚动面、SkillTips 弹窗关闭链和转生过场弹窗，并作为当前权威拓扑重新初始化。
