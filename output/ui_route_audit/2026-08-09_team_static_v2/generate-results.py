#!/usr/bin/env python3
"""Generate compact leaf results for the corrected Team v2 topology.

This route-local helper writes route-results.json only.  The official ledger
is updated exclusively through route_ledger.py apply.
"""
...
RUNTIME_LEAVES = {
    "mainui.team.view.hall.row-render": (
        ...
        "仍缺真实列表宿主、Prefab GraphicRaycaster、异步头像、field-scene 附近语义、列表复用、两档 viewport，"
...
def blocked_reason(item: dict) -> str:
    ...
    if item.get("risk") != "read-only":
        return (
            ...
            "用户要求真实写事务只枚举，本轮无账号写授权且页面 Prefab 缺失，"
            "因此未新增可执行点击绑定、未发协议。"
        )
    return (
...
def main() -> None:
    ...
    if len(results) != 102:
        raise SystemExit(f"unexpected corrected Team leaf count: {len(results)}")
